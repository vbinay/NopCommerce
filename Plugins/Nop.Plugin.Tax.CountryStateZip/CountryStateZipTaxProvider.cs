...
namespace Nop.Plugin.Tax.CountryStateZip
{
    ...
    public class CountryStateZipTaxProvider : BasePlugin, ITaxProvider
    {
        ...
        public CalculateTaxResult GetTaxRate(CalculateTaxRequest calculateTaxRequest)
        {
            ...
            var address = calculateTaxRequest.ShipToAddress;

            if (calculateTaxRequest.ShipToAddress == null)
            {
                address = calculateTaxRequest.Customer.BillingAddress;
                return result;
            }
            else
            {
                address = calculateTaxRequest.ShipToAddress;
            }
            ...
            int countryId = address.CountryId.GetValueOrDefault(1); //TODO REMOVE THE VALUEORDEFAULT(1)
            int stateProvinceId = address.StateProvince != null ? address.StateProvince.Id : 0;
            string zip = address.ZipPostalCode;
            ...
            {
                if (taxRate.CountryId == countryId && taxRate.TaxCategoryId == 3)
            ...
            if (!matchedByZip.Any())	
                foreach (var taxRate in matchedByStateProvince)
                    if (String.IsNullOrWhiteSpace(taxRate.Zip) || taxRate.Zip == "*")
                        matchedByZip.Add(taxRate);

            if (matchedByZip.Any())	
            {
                /// SODMYWAY-2955 - Added for loop to add multipe percentages to the Tax Rate.  Originally it was just the first percentage [matchedByZip[0].Percentage]
                for(int i = 0; i < matchedByZip.Count; i++)
                {
                    result.TaxRate += matchedByZip[i].Percentage;
                }
            }
        ...
        //public void PostDistributiveTaxRequest(Nop.Services.Orders.OrderProcessingService.PlaceOrderContainter details, Core.Domain.Orders.Order order, Nop.Services.Logging.ILogger logger, ITaxCategoryService tcService)
        //{
        //    //throw new NotImplementedException();
        //}

        public void PostQuotationTaxRequestinBulk(IList<Core.Domain.Orders.ShoppingCartItem> cart, Nop.Services.Logging.ILogger logger, ITaxCategoryService service)
        {
            //throw new NotImplementedException();
        }


        //public List<Core.Domain.Tax.DynamicVertexGLCode> GetDynamicGLCodes(int orderId)
        //{
        //    throw new NotImplementedException();
        //}


        public void PostDistributiveTaxRefundRequest(Core.Domain.Orders.Order order, int orderItemId, Nop.Services.Logging.ILogger logger,bool partialRefund, decimal AmountToRefund1 = 0, decimal AmountToRefund2 = 0, decimal AmountToRefund3 = 0, int ProductId = 0, List<int> data = null)
        {
            //notta
        }
        ...
        public void PostDistributiveTaxRequest(OrderProcessingService.PlaceOrderContainter details, Order order, ILogger logger, ITaxCategoryService tcService, IGlCodeService glService)
        {
