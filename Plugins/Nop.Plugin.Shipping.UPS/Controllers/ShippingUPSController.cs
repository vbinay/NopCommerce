...
namespace Nop.Plugin.Shipping.UPS.Controllers
{
    ...
    public class ShippingUPSController : BasePluginController
    {
        ...
        public ActionResult Configure()
        {
            ...
            model.ShipmentAcceptUrl = _upsSettings.ShipmentAcceptUrl;
            model.ShipmentConfirmUrl = _upsSettings.ShipmentConfirmUrl;
            ...
            foreach (var store in _storeService.GetAllStores().Select(s => new SelectListItem { Value = s.Id.ToString(), Text = s.Name }).ToList())
            {
                model.Stores.Add(new SelectListItem()
                {
                    Text = store.Text,
                    Value = store.Value,
                    Selected = store.Value == _storeContext.CurrentStore.Id.ToString()
                });
        ...
        public ActionResult Configure(UPSShippingModel model)
        {
            ...
            _upsSettings.ShipmentAcceptUrl = model.ShipmentAcceptUrl;
            _upsSettings.ShipmentConfirmUrl = model.ShipmentConfirmUrl;
            ...
            _upsSettings.SelectedStore = model.SelectedStore;
            ...
            _settingService.SaveSetting(_upsSettings, Convert.ToInt32(_upsSettings.SelectedStore));
