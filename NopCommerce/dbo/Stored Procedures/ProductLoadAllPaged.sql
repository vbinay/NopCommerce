...
(
	...
	@VendorId			int = 0,
	@WarehouseId		int = 0,
	@MasterProductId	int = 0, --- SODMYWAY-2945
	...
	@SearchManufacturerPartNumber bit = 0, -- a value indicating whether to search by a specified "keyword" in manufacturer part number	--- SODMYWAY-3297
	...
	@OverrideHideMasterProducts	bit = 0, --- SODMYWAY-2945
...
BEGIN
	...
	DECLARE
		@SearchKeywords bit,
		@OriginalKeywords nvarchar(4000),	--- SODMYWAY-3297
		@sql nvarchar(max),
	...
	SET @OriginalKeywords = @Keywords	--- SODMYWAY-3297
	IF ISNULL(@Keywords, '') != ''
	BEGIN
		...
		BEGIN
			...
			SELECT p.Id
			FROM Product p with (NOLOCK)
			WHERE p.[Sku] = @OriginalKeywords '	--- SODMYWAY-3297
		END
		...
		BEGIN
			...
			WHERE pt.[Name] = @OriginalKeywords '	--- SODMYWAY-3297
			...
			WHERE
				...
				AND lp.LanguageId = ' + ISNULL(CAST(@LanguageId AS nvarchar(max)), '0') + '
				AND lp.LocaleKey = N''Name''	--- SODMYWAY-3297
				AND lp.[LocaleValue] = @OriginalKeywords '	--- SODMYWAY-3297
		END

		-- PRINT (@sql)
		EXEC sp_executesql @sql, N'@Keywords nvarchar(4000), @OriginalKeywords nvarchar(4000)', @Keywords, @OriginalKeywords	--- SODMYWAY-3297
	...
	IF (@OverrideHideMasterProducts = 1) --Hide Master Products?
	BEGIN
		--show master products only
		SET @sql = @sql + '
		AND p.IsMaster = 0'
	END
	ELSE IF (@OverrideHideMasterProducts = 0)
	BEGIN
		--only local products --JIRA - 2945
		SET @sql = @sql + '
		AND p.IsMaster = 1'
	END
	--- #endregion SODMYWAY-2945
	...
	--show hidden and ACL
	IF  @ShowHidden = 0 and @FilteredCustomerRoleIdsCount > 0	--- SODMYWAY-3297
	BEGIN
		SET @sql = @sql + '
	...
	BEGIN
		SET @sql = @sql + '
		AND (EXISTS (	--- SODMYWAY-2945
			SELECT 1 FROM [StoreMapping] sm with (NOLOCK)
			WHERE [sm].EntityId = p.Id AND [sm].EntityName = ''Product'' and [sm].StoreId=' + CAST(@StoreId AS nvarchar(max)) + '
			))'
	END
	ELSE  --- SODMYWAY-2945
	SET @sql = @sql + '	--- SODMYWAY-2945
		AND P.IsMaster = 1'	--- SODMYWAY-2945
	...
	SET @SpecAttributesCount = (SELECT COUNT(1) FROM #FilteredSpecsWithAttributes)	--- SODMYWAY-3297
	...
	--- #endregion
	PRINT (@sql)	--- SODMYWAY-2945
	EXEC sp_executesql @sql

	DROP TABLE #FilteredCategoryIds
	DROP TABLE #FilteredSpecs
    DROP TABLE #FilteredSpecsWithAttributes	--- SODMYWAY-3297
