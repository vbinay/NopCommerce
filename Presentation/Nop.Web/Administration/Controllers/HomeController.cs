...
namespace Nop.Admin.Controllers
{
    public partial class HomeController : BaseAdminController
    {
        ...
        public ActionResult CommonStatistics()
        {
            ...
            model.NumberOfOrders = _orderService.SearchOrders(storeId: _storeContext.CurrentStore.Id,  // NU-42
                pageIndex: 0,
            ...
            foreach(var order in orders)
            {
                if (order.OrderStatus != OrderStatus.Complete && order.OrderStatus != OrderStatus.Cancelled)
                {
                    ...
                    foreach (var orderItem in order.OrderItems)
                    {
                        ...
                        {
                            model.NumberOfOutstandingFulfillments ++;
                        }
                    }
                }
            }

            model.NumberOfCustomers = _customerService.GetAllCustomers(storeId: _storeContext.CurrentStore.Id,	// NU-42
                customerRoleIds: new[] { _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Registered).Id },
                pageIndex: 0,
                pageSize: 1).TotalCount;

            //model.NumberOfPendingReturnRequests = _returnRequestService.SearchReturnRequests(storeId: _storeContext.CurrentStore.Id,	// NU-42
            //    rs: ReturnRequestStatus.Pending,
            //    pageIndex: 0,
            //    pageSize: 1).TotalCount;

            model.NumberOfLowStockProducts = _productService.GetLowStockProducts(0, 0, 1, _storeContext.CurrentStore.Id).TotalCount +
                                             _productService.GetLowStockProductCombinations(0, 0, 1, _storeContext.CurrentStore.Id).TotalCount;
