...
namespace Nop.Admin.Controllers
{
    public partial class MessageTemplateController : BaseAdminController
    {
        ...
        public ActionResult List(DataSourceRequest command, MessageTemplateListModel model)
        {
            ...
            var messageTemplates = _messageTemplateService.GetAllMessageTemplates(_storeContext.CurrentStore.Id);	// NU-42
...
foreach (var ea in _emailAccountService.GetAllEmailAccounts())
        ...
        public ActionResult TestTemplate(int id, int languageId = 0)
        {
            ...
            tokens = tokens.Where(x => subject.Contains(x) || body.Contains(x) || (messageTemplate.IsToKitchenPrinter && x.IndexOf("KitchenPrinter") > -1)).ToList();	// NU-84
            ...
            defaultValues.Add("%Store.Name%", _storeContext.CurrentStore.Name);
            //defaultValues.Add("%Store.Email%", _storeContext.CurrentStore.em);
            defaultValues.Add("%Order.OrderNumber%", order.Id);
            defaultValues.Add("%Order.BillingFirstName%", order.BillingAddress.FirstName);
            defaultValues.Add("%Order.BillingLastName%", order.BillingAddress.LastName);
            defaultValues.Add("%Order.BillingPhoneNumber%", order.BillingAddress.PhoneNumber);
            defaultValues.Add("%Order.BillingEmail%", order.BillingAddress.Email);
            defaultValues.Add("%Order.BillingAddress1%", order.BillingAddress.Address1);
            defaultValues.Add("%Order.BillingAddress2%", order.BillingAddress.Address2);
            defaultValues.Add("%Order.ShippingFirstName%", order.ShippingAddress.FirstName);
            defaultValues.Add("%Order.ShippingLastName%", order.ShippingAddress.LastName);
            defaultValues.Add("%Order.ShippingPhoneNumber%", order.ShippingAddress.PhoneNumber);
            defaultValues.Add("%Order.ShippingEmail%", order.ShippingAddress.Email);
            defaultValues.Add("%Order.ShippingAddress1%", order.ShippingAddress.Address1);
            defaultValues.Add("%Order.ShippingAddress2%", order.ShippingAddress.Address2);
            defaultValues.Add("%Store.CompanyAddress%", _storeContext.CurrentStore.CompanyAddress);
            defaultValues.Add("%Store.CompanyPhoneNumber%", _storeContext.CurrentStore.CompanyPhoneNumber);
            defaultValues.Add("%Order.CreatedOn%", DateTime.Now);
            defaultValues.Add("%KitchenPrinter.URL%", "http://10.102.1.30/cgi-bin/epos/service.cgi?devid=local_printer&timeout=10000");

            model.SendTo = _workContext.CurrentCustomer.Email;
            model.DefaultValues = defaultValues;
			#endregion
        ...
        public ActionResult TestTemplate(TestMessageTemplateModel model, FormCollection form)
        {
			...
			#region NU-84
            if(messageTemplate.IsToKitchenPrinter)
            {
                string kitchenPrinterURL = form["token_%KitchenPrinter.URL%"];
                string orderNumber = form["token_%Order.OrderNumber%"];

                _workflowMessageService.SendTestReceipt(messageTemplate.Id, model.LanguageId, kitchenPrinterURL, orderNumber);
            }
            else
            {
