...
namespace Nop.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        {
            var allStores = _storeService.GetAllStores(_workContext.CurrentCustomer).OrderBy(s => s.Name);	/// NU-75
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    model.IsReadOnly = customer.ReadOnly;
            ...
            #region NU-57
            int[] internalRoles = { SystemCustomerRoleNames.AdministratorsId, SystemCustomerRoleNames.ForumModeratorsId, SystemCustomerRoleNames.GlobalAdministratorsId, SystemCustomerRoleNames.StoreAdministratorsId, SystemCustomerRoleNames.VendorId };
            var rolesQuery = from cr in model.SelectedCustomerRoleIds
                             where (from i in internalRoles
                                    select i).Contains(cr)
                             select cr;
            model.IsInternal = rolesQuery.Count() > 0;
            #endregion

            model.IsSystemGlobalAdmin = _workContext.CurrentCustomer.IsSystemGlobalAdmin(); // NU-61

            #region NU-62

            foreach (var store in allStores)
            {
                model.StoreMappingAvailableStores.Add(new SelectListItem
                {
                    Text = store.Name,
                    Value = store.Id.ToString(),
                    Selected = (store.Id == _storeContext.CurrentStore.Id)
        ...
        #region NU-57
        public ActionResult InternalList()
        {
            return List(true);
        }
        #endregion
            ...
            bool isInternal = false)      // NU-56
        {
            ...
            if (isInternal)
            {
                defaultRoleIds = new List<int> { _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.StoreAdministrators).Id };
            }
            #endregion
            ...
            var allVendors = _vendorService.GetAllVendors(showHidden: true);
            ...
            [ModelBinder(typeof(CommaSeparatedModelBinder))] int[] searchCustomerRoleIds)
        {
            ...
            if (!model.IsInternal)
            {
                model.SearchStoreId = _storeContext.CurrentStore.Id;  /// NU-42
            }
            #endregion
            ...
            var customers = _customerService.GetAllCustomers(
                ...
                storeId: model.SearchStoreId,	// NU-36
                vendorId: model.SearchVendorId);	/// NU-48

            var gridModel = new DataSourceResult
            {
                Data = customers.Select(PrepareCustomerModelForList)
                    .OrderByDescending(c => c.Id),  // NU-37
        ...
        public ActionResult Create(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _storeMappingService.InsertStoreMapping(customer, _storeContext.CurrentStore.Id); //add mapping for current store. nu-90
                ...
                _workflowMessageService.SendCustomerCreationNotification(customer, 1);
                if (continueEditing)
                {
                    //selected tab
                    SaveSelectedTabName();
        ...
        public ActionResult Edit(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    customer.ReadOnly = model.IsReadOnly;
        ...
        public ActionResult SendEmail(CustomerModel model)
        {
            ...
            try
            {
                ...
                var emailAccount = _storeService.GetStoreContactByEmailTypeId(1, _storeContext.CurrentStore.Id);
                ...
                Data = orders.PagedForCommand(command)
                    .Select(order =>
                    {
                        ...
                        var orderModel = new CustomerModel.OrderModel
                        {
                            ...
                            ProductCount = order.OrderItems.Count //SODMYWAY-3467
                        };
                        return orderModel;
                    }).OrderByDescending(c => c.CreatedOn), /// NU-37
                Total = orders.Count
            };
        ...
        public ActionResult ReportBestCustomersByOrderTotalList(DataSourceRequest command, BestCustomersReportModel model)
        {
            ...
            var gridModel = new DataSourceResult
            {
                Data = items.Select(x =>
                {
                    ...
                    return m;
                }).OrderByDescending(c => c.OrderTotal), /// NU-47
        ...
        public ActionResult ReportBestCustomersByNumberOfOrdersList(DataSourceRequest command, BestCustomersReportModel model)
        {
            ...
            var gridModel = new DataSourceResult
            {
                Data = items.Select(x =>
                {
                    ...
                    return m;
                }).OrderByDescending(c => c.OrderCount), /// NU-47
                Total = items.TotalCount
            };
                    ...
                    if (!timeZone.IsInvalidTime(searchYearDateUser))
                    {
                        ...
                        for (int i = 0; i <= 12; i++)
                        {
                            result.Add(new
                            {
                                ...
                                value = _customerService.GetAllCustomers(storeId: _storeContext.CurrentStore.Id,
        ...
        #region NU-56
        [HttpPost, ActionName("InternalList")]
        [FormValueRequired("exportexcel-all")]
        public ActionResult ExportExcelAllInternal(CustomerListModel model)
        {
            return ExportExcelAll(model, true);
        }
        #endregion
            ...
            bool isInternal = false) // NU-56
        {
            ...
            var customers = _customerService.GetAllCustomers(
                ...
                zipPostalCode: model.SearchZipPostalCode,
                loadOnlyWithShoppingCart: false,
                storeId: model.SearchStoreId)   // NU-56
                .OrderBy(c => c.Email).ToList();  // NU-44
        ...
        public ActionResult ExportXmlAllInternal(CustomerListModel model)
        {
            return ExportXmlAll(model, true);
        }
        #endregion
        ...
        #region NU-27
        [NonAction]
        protected virtual void SyncVendorCustomerMappings(Customer customer)
        {
            if (customer.VendorId > 0)
            {
                var vendor = _vendorService.GetVendorById(customer.VendorId);

                var vendorCustomers = _customerService.GetCustomersByVendor(vendor);

                List<Store> stores = new List<Store>();
                foreach (var vendorCustomer in vendorCustomers)
                {
                    var mappings = _storeMappingService.GetStoreMappings(vendorCustomer);

                    foreach (var mapping in mappings)
                    {
                        stores.Add(mapping.Store);
                    }
                }
        ...
        [HttpPost]
        public ActionResult StoreMappingList(int customerId)
        {
            ...
            var storeMappingModel = _storeMappingService.GetStoreMappings(customer).ToList();

            var gridModel = new DataSourceResult
            {
                Data = storeMappingModel,
                Total = storeMappingModel.Count()
            };
        ...
        [HttpPost]
        public ActionResult StoreMappingAdd(int customerId, int storeId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageStores))
                return AccessDeniedView();
            ...
            _storeMappingService.InsertStoreMapping(customer, storeId);
        ...
        [HttpPost]
        public ActionResult StoreMappingDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageStores))
                return AccessDeniedView();
            ...
            var mapping = _storeMappingService.GetStoreMappingById(id);
