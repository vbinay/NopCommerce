...
namespace Nop.Admin.Controllers
{
    public partial class SettingController : BaseAdminController
	{
        ...
        public ActionResult SAP()
        {
            ...
            var settings = _settingService.LoadSetting<SAPSettings>();
            var model = settings.ToModel();
            model.Protocols = new SelectList(new[] {"FTP", "SFTP"});
            model.TestOutput = _SAPService.Count();

            return View(model);
        }

        [HttpPost]
        public ActionResult SAP(SAPSettingsModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageSettings))
                return AccessDeniedView();
