...
var Accordion = {
    ...
    openSection: function (section) {
        ...
        if (section.attr('id') != this.currentSectionId) {
            ...
            $(contents[0]).show();
            $(".checkout-data").attr('aria-valuenow', $('#' + this.currentSectionId + ' span.number').text());
            $(".checkout-data").attr('aria-valuetext', $('#' + this.currentSectionId + ' h2.title').text());
            $('#' + this.currentSectionId + ' .step-title a').focus();
            ...
            if (this.disallowAccessToNextSections) {
                ...
                for (var i = 0; i < this.sections.length; i++) {
                    if (pastCurrentSection) {
                        ...
                        var $i = $(this.sections[i]).attr('id');
                        $("#" + $(this.sections[i]).attr('id') + ' a').attr('aria-disabled', true);
                        $("#" + $(this.sections[i]).attr('id') + ' a').attr('tabindex', '-1');
                    }
