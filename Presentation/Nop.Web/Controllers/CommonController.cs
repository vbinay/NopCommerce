...
namespace Nop.Web.Controllers
{
    public partial class CommonController : BasePublicController
    {
        ...
        private readonly IStoreService _storeService; //NU 90
        ...
        public ActionResult HeaderLinks()
        {
            ...
            #region SODMYWAY-
            var isAuth = customer.IsRegistered() && (
                _workContext.CurrentCustomer.IsSystemGlobalAdmin()
                || (_customerService.GetCustomerByIdAndStoreId(customer.Id, _storeContext.CurrentStore.Id) != null));
            #endregion

            var customerNameToDisplay = customer.BillingAddress != null ? customer.BillingAddress.FirstName + " " + customer.BillingAddress.LastName : customer.Username;


            var model = new HeaderLinksModel
            {
                IsAuthenticated = isAuth,	/// SODMYWAY-
                CustomerEmailUsername = customer.IsRegistered() ? customerNameToDisplay : "",
                ShoppingCartEnabled = _permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart),
                WishlistEnabled = _permissionService.Authorize(StandardPermissionProvider.EnableWishlist),
                AllowPrivateMessages = customer.IsRegistered() && _forumSettings.AllowPrivateMessages,
                AllowRegistrations = (_customerSettings.UserRegistrationType == UserRegistrationType.Disabled ? false : true), //NU-90
        ...
        public ActionResult AdminHeaderLinks()
        {
            ...
            #region SODMYWAY-
            var disAdmLink = _permissionService.Authorize(StandardPermissionProvider.AccessAdminPanel);
            disAdmLink = disAdmLink && (
                _workContext.CurrentCustomer.IsSystemGlobalAdmin()
                || (_customerService.GetCustomerByIdAndStoreId(customer.Id, _storeContext.CurrentStore.Id) != null));
            #endregion
            ...
            var model = new AdminHeaderLinksModel
            {
                ...
                ImpersonatedCustomerEmailUsername = customer.IsRegistered() ? customerNameToDisplay : "",
                IsCustomerImpersonated = _workContext.OriginalCustomerIfImpersonated != null,
                DisplayAdminLink = disAdmLink,	/// SODMYWAY-
        ...
        public ActionResult Footer()
        {
            ...
            var model = new FooterModel
            {
                ...
                IsLoggedIn = !this._workContext.CurrentCustomer.IsGuest(),
        ...
        {
            //validate CAPTCHA
            CaptchaResponse response = ValidateCaptcha(Request.Params["captcha-response"]);
            if (_captchaSettings.Enabled && _captchaSettings.ShowOnContactUsPage && !response.Success)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                var storeContact = _storeService.GetStoreContactByEmailTypeId(1, _storeContext.CurrentStore.Id);
                ...
                body = "User email: " + email + "<br />" + body;

                //required for some SMTP servers
                if (_commonSettings.UseSystemEmailForContactUsForm)
                {
                    from = storeContact.Email;
                    fromName = storeContact.DisplayName;
                ...
                _queuedEmailService.InsertQueuedEmail(new QueuedEmail
                {
                    ...
                    ToName = storeContact.DisplayName,
                    ReplyTo = email,
                ...
                _logger.InsertLog(LogLevel.Information, "Contact Us", string.Format("From:{0}, FromName:{1}, To:{2}, ToName:{3},ReplyTo:{4},ReplyToName:{5}, Subject{6}, Body:{7}", from, fromName, storeContact.Email, storeContact.DisplayName, email, fullName, subject, body));
        ...
        public ActionResult RobotsTextFile()
        {
            ...
            else
            {
                ...
                var localizableDisallowPaths = new List<string>
                {
                    ...
                    "/addproductstocart/catalog/",	/// SODMYWAY-
        ...
        [ChildActionOnly]
        public ActionResult BannerHead()
        {
            HtmlAgilityService service = new HtmlAgilityService();
            BannerHeadModel model = new BannerHeadModel();
            var memoryCache = MemoryCache.Default;

            string baseUrl = _storeContext.CurrentStore.DiningSiteUrl;


            if (String.IsNullOrEmpty(_storeContext.CurrentStore.DiningSiteUrl))
            {
                if (!String.IsNullOrEmpty(WebConfigurationManager.AppSettings["DefaultBannerURL"]))
                {
                    baseUrl = WebConfigurationManager.AppSettings["DefaultBannerURL"];
                }
                else
                {
                    baseUrl = "";
            ...
            else
            {
                ...
                if (_storeContext.CurrentStore.DiningSiteUrl.Contains("shop-"))
                {
                    if (!String.IsNullOrEmpty(WebConfigurationManager.AppSettings["DefaultBannerURL"]))
                    {
                        baseUrl = WebConfigurationManager.AppSettings["DefaultBannerURL"];
                ...
                {
                    baseUrl = _storeContext.CurrentStore.DiningSiteUrl;
                }


            }

            string memCacheData = (string)memoryCache.Get("baseUrl");
            if (memCacheData != null)
            {
                if (memCacheData != baseUrl)
                {
                    memoryCache.Remove("bannerCache");
                    memoryCache.Remove("baseUrl");
                    
                }
            }

            HtmlAgilityService agility = new HtmlAgilityService();
            List<string> head = new List<string>();
            //var keyName = $"Header{_storeContext.CurrentStore.Id}";
            //if (_cacheManager.IsSet(keyName))
            //{
            //    model.Head = _cacheManager.Get<List<string>>(keyName);
            //}
            //else 
            //{
            //    try
            //    {
            //        var headResult = agility.GetHead(baseUrl).Result;
            //        if (headResult != null)
            //        {
            //            model.Head = headResult;
            //            _cacheManager.Set(keyName, head, 15);
            //        }
            //        else
            //        {
            //            model.Head = head;
            ...
            //if (agility.GetHead(baseUrl).Result != null)
            //{

            //Caching Implementaition
            //if (!memoryCache.Contains("bannerCache"))
            //{
            //    var expiration = DateTimeOffset.UtcNow.AddMinutes(5);
            //    head = agility.GetHead(baseUrl).Result;
            //    memoryCache.Add("baseUrl", baseUrl, expiration);
            //    memoryCache.Add("bannerCache", head, expiration);
            //}
            //List<string> Headercollection = (List<string>)memoryCache.Get("bannerCache", null);
            //model.Head = Headercollection;
        ...
        public ActionResult BannerBar()
        {
            ...
            string shopUrl = _storeContext.CurrentStore.SecureUrl;


            if (!String.IsNullOrEmpty(_storeContext.CurrentStore.DiningSiteUrl) && !(_storeContext.CurrentStore.DiningSiteUrl.ToLower().Contains("shop-")))
            {
                if (_storeContext.CurrentStore.DiningSiteUrl.ToLower().Contains("-preview"))
                {
                    baseUrl = _storeContext.CurrentStore.DiningSiteUrl;
                    //  baseUrl = _storeContext.CurrentStore.DiningSiteUrl.ToLower().Replace("-preview", "");
                }
                else
                {
                    baseUrl = _storeContext.CurrentStore.DiningSiteUrl;
            ...
            else
            {
                ...
                string httpHost = Convert.ToString(webHelper.ServerVariables("HTTP_HOST"));
                if (httpHost.ToLower().Contains("shop-"))
                    baseUrl = httpHost.Replace("shop-", "");
                else
                    baseUrl = httpHost;
            }
            ...
            head = agility.GetHeader(baseUrl, shopUrl).Result;
