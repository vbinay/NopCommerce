...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        private readonly IOrderService _orderService; //EC-5
        private readonly ICustomerService _customerService; //EC-5
        ...
        public ProductController(ICategoryService categoryService,
            ...
            IShippingService shippingService, //EC-5
            ...
            ICacheManager cacheManager)
        {
            ...
            this._orderService = orderService; //EC-5
            this._productAttributeParser = productAttributeParser;
            this._shippingService = shippingService;
            this._mealPlanService = mealplanService;
            this._cardAccessService = cardAccessService;
        ...
        public JsonResult GetAvailableSlot(int productId, string selectedDate, string timeSlot)
        {
            var sb = new StringBuilder();
            IList<ProductFulfillmentCalendar> excludedDates = _productService.GetUnavlblDates(productId, DateTime.Now, DateTime.Now.Date.AddDays(2));
            DateTime selectedDt = (DateTime)_dateTimeHelper.ConvertToUtcTime(DateTime.Parse(selectedDate), _dateTimeHelper.CurrentTimeZone);

            int avlblUnits = 0;
            bool IsSelecteddateUnavlbl = false;

            if (excludedDates.Any())
            {
                foreach (ProductFulfillmentCalendar calDay in excludedDates)
                {
                    DateTime dateToCompare = (DateTime)_dateTimeHelper.ConvertToUtcTime(DateTime.Parse(calDay.Date.ToString()), _dateTimeHelper.CurrentTimeZone);
                    if (dateToCompare == selectedDt)
                    {
                        IsSelecteddateUnavlbl = true;
                        break;
                    }
                }
            }
            if(!IsSelecteddateUnavlbl)
            {
                var reservationProducts = _productService.GetReservationProductsbyProductId(productId);
                if (String.IsNullOrEmpty(timeSlot))
                {
                    timeSlot = reservationProducts.FirstOrDefault().TimeSlotsConfigured;
                }
                int occupancyConfiguredForSlot = reservationProducts.Where(x => x.TimeSlotsConfigured == timeSlot).FirstOrDefault().OccupancyUnitsAvailable ;
                var reservedProducts = _productService.GetReservedProductbyProductId(productId);
                int unitsAlreadyReservedForSlot = reservedProducts.Where(x => x.ReservationDate.ToShortDateString() == selectedDt.ToShortDateString() && x.ReservedTimeSlot == timeSlot).Select(it => it.ReservedUnits).Sum();
                avlblUnits = occupancyConfiguredForSlot - unitsAlreadyReservedForSlot;
            }
            return Json(avlblUnits, JsonRequestBehavior.AllowGet);
        }

        #region SODMYWAY-
        [HttpPost]
        public ActionResult ValidateCardSystemAccount(string accountNumber)
        {
            ...
            string issuerId = "1";
            string planId = "4";
            string applicationId = "000001";

            CardAccessRecord record = this._cardAccessService.GetBalance(seed, issuerId, accountNumber, planId, applicationId);

            return Json(new { Result = true, Error = record.Error, Balance = record.Balance }, JsonRequestBehavior.AllowGet);
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            return this.PrepareProductOverviewModels(_workContext,
                ...
                _pictureService, _measureService, _orderService, _webHelper, _cacheManager,
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                var defaultPictureModel = new PictureModel
                {
                    ...
                    AlternateText = (!string.IsNullOrEmpty(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"))) ? string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name) : model.Name,
                };
                //"title" attribute
                ...
                defaultPictureModel.AlternateText = (defaultPicture != null && !string.IsNullOrEmpty(defaultPicture.AltAttribute)) ?
                    ...
                    ((!string.IsNullOrEmpty(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"))) ? string.Format(_localizationService.GetResource("Media.Product.ImageAlternateTextFormat.Details"), model.Name) : model.Name);
            ...
            model.AddToCart.IsReservation = product.IsReservation;
            ...
            var limitPurchaseResponse = ControllerExtensions.LimitPurchase(product, _productService, _orderService, _storeContext, _workContext);
            model.AddToCart.LimitPurchaseResult = limitPurchaseResponse;

            bool disableBuy = _workContext.CurrentCustomer.IsGuest() && product.DisableBuyButtonForGuestUsers;

            //'add to cart', 'add to wishlist' buttons
            //model.AddToCart.DisableBuyButton = product.IsBundleProduct ||product.DisableBuyButton || disableBuy || limitPurchaseResponse.LimitPurchase || !_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart);
            model.AddToCart.DisableBuyButton = product.DisableBuyButton || disableBuy || limitPurchaseResponse.LimitPurchase || !_permissionService.Authorize(StandardPermissionProvider.EnableShoppingCart);
            ...
            if (product.AvailableForPreOrder)
            {
                model.AddToCart.AvailableForPreOrder = product.PreOrderAvailabilityStartDateTimeUtc.HasValue &&
                    product.PreOrderAvailabilityStartDateTimeUtc.Value >= DateTime.UtcNow && product.AvailableForPreOrder;
            ...
            #region NU-16
            model.MealPlan.IsMealPlan = product.IsMealPlan;
            model.MealPlan.ShowStandardMealPlanFields = product.ShowStandardMealPlanFields;



            //           fsuedufirstname
            //uid
            //fsuedupreferredemail
            //fsuedulastname
            //fsueduemplid
            ...
            if (model.MealPlan.IsMealPlan)
            {

                if (updatecartitem == null)
                {
                    //model.MealPlan.SenderName = _workContext.CurrentCustomer.GetFullName();
                    //model.GiftCard.SenderEmail = _workContext.CurrentCustomer.Email;
                }
                else if (product.ShowStandardMealPlanFields)
                {
                    ...
                    _productAttributeParser.GetMealPlanAttribute(updatecartitem.AttributesXml,
                    ...
                    model.MealPlan.MealPlanRecipientAcctNum = mealPlanRecipientAcctNum;
                    model.MealPlan.MealPlanRecipientAddress = mealPlanRecipientAddress;
                    model.MealPlan.MealPlanRecipientEmail = mealPlanRecipientEmail;
                    model.MealPlan.MealPlanRecipientName = mealPlanRecipientName;
                    model.MealPlan.MealPlanRecipientPhone = mealPlanRecipientPhone;
            ...
            #region NU-17
            model.Donation.IsDonation = product.IsDonation;
            if (model.Donation.IsDonation)
            {

                model.Donation.ShowOnBehalfOf = true;
            ...
            foreach (var attribute in productAttributeMapping)
            {
                ...
                ////NU-90 -- TODO: REIMPLIMENT IF NEEDED
                //if (attribute.ProductAttribute.Name == "FSUID")
                //{
                //    attributeModel.DefaultValue =_workContext.CurrentCustomer.GetAttribute<String>("uid", _storeContext.CurrentStore.Id);
                //}
                //else if (attribute.ProductAttribute.Name == "FSUCard #")
                //{

                //    attributeModel.DefaultValue = "";
                //}
                //else if (attribute.ProductAttribute.Name == "Account ID Number")
                //{
                //    attributeModel.DefaultValue =_workContext.CurrentCustomer.GetAttribute<String>("fsueduemplid", _storeContext.CurrentStore.Id);
                //}
                ////NU-90
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        if (_permissionService.Authorize(StandardPermissionProvider.DisplayPrices))
                        {
                            ...
                            if (priceAdjustmentBase > decimal.Zero && !model.IsBundleProduct)
                            {
                                valueModel.PriceAdjustment = "+" + _priceFormatter.FormatPrice(priceAdjustment, false, false);
                            }
                            else if (priceAdjustmentBase < decimal.Zero && !model.IsBundleProduct)
                            {
                                valueModel.PriceAdjustment = "-" + _priceFormatter.FormatPrice(-priceAdjustment, false, false);
                            }
            ...
            var productAttributesModifier = model.ProductAttributes.Where(x => x.Values.Count == combinations.Count && x.AttributeControlType == AttributeControlType.DropdownList).ToList();

            if (productAttributesModifier.Any())
            {
                int count = 0;
                foreach (var itemdata in combinations)
                {
                    if (itemdata.StockQuantity <= 0)
                    {
                        productAttributesModifier[productAttributesModifier.Count - 1].Values[count].Name = productAttributesModifier[productAttributesModifier.Count - 1].Values[count].Name + " " + "Temporarily out of stock";
            ...
            #region Reservation Products

            ReservationProductDetailsModel reservationProductDetailsModel = new ReservationProductDetailsModel();
            reservationProductDetailsModel.ProductId = product.Id;
            reservationProductDetailsModel.ReservationCapPerSlot = product.ReservationCapPerSlot;

            if (product.IsReservation)
            {
                model.IsReservation = true;

                if (model.ReservationProductDetailsModel == null)
                {
                    model.ReservationProductDetailsModel = new ReservationProductDetailsModel();
                    List<ReservationProducts> reservationProducts = new List<ReservationProducts>();
                    reservationProducts = _productService.GetReservationProductsbyProductId(product.Id).ToList();
                    var reservedProducts = _productService.GetReservedProductbyProductId(product.Id);

                    #region Calculate avl Units based on timeslot
                    Dictionary<string, int> UnitsAvlPerTimeSlot = new Dictionary<string, int>();
                    if (reservationProducts.Count > 0)
                    {
                        UnitsAvlPerTimeSlot.Clear();
                        foreach (var reservationProduct in reservationProducts)
                        {
                            if (!UnitsAvlPerTimeSlot.ContainsKey(reservationProduct.TimeSlotsConfigured))
                            {
                                UnitsAvlPerTimeSlot.Add(reservationProduct.TimeSlotsConfigured, reservationProduct.OccupancyUnitsAvailable);
                            }
                        }
                    }
                    reservationProductDetailsModel.OccupancyUnitsAvailable = UnitsAvlPerTimeSlot.FirstOrDefault().Value;
                    if (reservedProducts.Count > 0)
                    {
                        foreach (var item in UnitsAvlPerTimeSlot.ToList())
                        {
                            int totalReservedUnits;
                            totalReservedUnits = reservedProducts.Where(x => x.ReservedTimeSlot == item.Key).Sum(x => x.ReservedUnits);
                            int UnitsAvl = item.Value - totalReservedUnits;
                            if (UnitsAvl > 0)
                            {
                                UnitsAvlPerTimeSlot[item.Key] = UnitsAvl;
                            }
                        }
                    }
                    #endregion

                    foreach (var item in UnitsAvlPerTimeSlot)
                    {
                        SelectListItem si = new SelectListItem();
                        si.Text = item.Key;
                        si.Value = item.Key;
                        reservationProductDetailsModel.TimeslotsConfigured.Add(si);
                    }

                    reservationProductDetailsModel.OccupancyUnitsAvailableForReservation = UnitsAvlPerTimeSlot.FirstOrDefault().Value;
                    reservationProductDetailsModel.ReservationCapPerSlot = product.ReservationCapPerSlot;

                    #region set unavailable calendar days
                    var sb = new StringBuilder();
                    DateTime minDate = DateTime.Now;
                    DateTime maxDate = DateTime.Now.AddDays(3);

                    var excludedDates = _productService.GetUnavlblDates(product.Id, minDate, maxDate);

                    foreach (var exclCalDay in excludedDates)
                    {
                        if (exclCalDay.Date.Date == minDate)
                        {
                    ...
                    reservationProductDetailsModel.JsExcludedDatesStr = sb.ToString();
                    reservationProductDetailsModel.JsMinDateStr = string.Format("{0},{1},{2},{3},{4}", minDate.Year, minDate.Month - 1, minDate.Day, minDate.Hour, minDate.Minute);
                    reservationProductDetailsModel.JsMaxDateStr = string.Format("{0},{1},{2}", maxDate.Year, maxDate.Month - 1, maxDate.Day);

                    #endregion

                    model.ReservationProductDetailsModel = reservationProductDetailsModel;
                }
            }
            #endregion
            return model;
        ...
        [ChildActionOnly]
        public ActionResult CrossSellProducts(int productId, int? productThumbPictureSize)
        {
            //var cart = _workContext.CurrentCustomer.ShoppingCartItems
            ...
            //var productIds = _cacheManager.Get(string.Format(ModelCacheEventConsumer.PRODUCTS_RELATED_IDS_KEY, productId, _storeContext.CurrentStore.Id),
            //    () =>
            //        _productService.GetCrossSellProductsByProductId1(productId).Select(x => x.ProductId2).ToArray()
            //        );
            var productIds = _productService.GetCrossSellProductsByProductId1(productId).Select(x => x.ProductId2).ToArray();
            //load products
            var products = _productService.GetProductsByIds(productIds);
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
