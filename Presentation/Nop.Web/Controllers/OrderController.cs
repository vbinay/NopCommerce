...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        private readonly ICardAccessService _cardAccessSystem; 
            ...
            IRepository<OrderItem> orderItemRepository)
        {
            ...
            this._cardAccessSystem = cardAccessService; 
        ...
        protected virtual OrderDetailsModel PrepareOrderDetailsModel(Order order)
        {
            ...
            foreach (var orderItem in orderItems)
            {
                if (orderItem.IsBundleProduct)
                {
                    ...
                    {
                        upriceInctx += _orderService.GetOrderItemById(sc.OrderItemId).UnitPriceInclTax;
                        upriceExcTax += _orderService.GetOrderItemById(sc.OrderItemId).UnitPriceExclTax;
                        priceInctx += _orderService.GetOrderItemById(sc.OrderItemId).PriceInclTax;
                        priceExcTax += _orderService.GetOrderItemById(sc.OrderItemId).PriceExclTax;
                    }
                    ...
                    if (model.Items.Where(x => x.ProductId == parentProductId).Any())
                    {
                        continue;
                    }
                }
                ...
                ReservedProduct resProduct = new ReservedProduct();
                resProduct = _productService.GetReservedProductbyOrderItemId(orderItem.Id);
                var orderItemModel = new OrderDetailsModel.OrderItemModel
                {
                    ...
                    CheckoutNotes = orderItem.Product.CheckoutNotes,    /// NU-19
                    SelectedShippingMethodName = orderItem.SelectedShippingMethodName, //SODMYWAY-
                    RequestedFulfillmentDateTime = orderItem.RequestedFulfillmentDateTime.GetValueOrDefault(),//SODMYWAY-
                    SelectedFulfillmentWarehouseId = orderItem.SelectedFulfillmentWarehouseId,//SODMYWAY-
                    IsReservation = orderItem.IsReservation
                };
                orderItemModel.ReservedProduct = resProduct;

                orderItemModel.ReservedProduct = resProduct;
                if (orderItemModel.SelectedFulfillmentWarehouseId != null && orderItemModel.SelectedFulfillmentWarehouseId > 0)//SODMYWAY-
                {
                    orderItemModel.SelectedFulfillmentWarehouse = _shippingService.GetWarehouseById(orderItemModel.SelectedFulfillmentWarehouseId.GetValueOrDefault(-1));
                }
        ...
        //My account / Orders
        [NopHttpsRequirement(SslRequirement.Yes)]
        public ActionResult CustomerMealPlans()
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            var model = new CustomerMealPlanListModel();
      

            List<MealPlan> list = _mealplanService.GetMealPlans(_storeContext.CurrentStore.Id, customer.Id, null, true, null, null, false);
            foreach (var mealplan in list)
            {
                model.MealPlans.Add(new CustomerMealPlanListModel.MealPlanDetailsModel
                {
                    MealPlanName = mealplan.PurchasedWithOrderItem.Product.Name,
                    ...
                    IsProcessed = mealplan.IsProcessed ? "Yes" : "No",
                    MealPlanAmount = mealplan.PurchasedWithOrderItem.PriceExclTax.ToString("F"),
                    CreatedOnLocal = mealplan.CreatedOnUtc.ToShortDateString(),
                    OrderId = mealplan.PurchasedWithOrderItem.Order.Id,
                    CardAccessRecordId = mealplan.CardAccessRecordId
            ...
            //List<CardAccessRecord> cardAccessRecords = _cardAccessSystem.GetAccounts(_workContext.CurrentCustomer.Id);

            //foreach(CardAccessRecord cas in cardAccessRecords)
            //{
            //    model.CardAccessSystemRecords.Add(new CustomerMealPlanListModel.CardAccessSystemRecordModel
            //    {
            //        Id = cas.Id,
            //        AccountId = cas.AccountId,
            //        IssuerId = cas.IssuerId,
            //        PlanId = cas.PlanId,                    
            //        ApplicationID = cas.ApplicationID,
            //        AppliedAmount = cas.AppliedAmount,
            //        Balance = cas.Balance,
            //        CardHolderID = cas.CardHolderID,
            //        CreatedOnUTC = cas.CreatedOnUTC,
