...
namespace Nop.Core.Domain.Customers
{
    public static class CustomerExtensions
    {
        ...
        #region NU-1
        public static bool IsSystemGlobalAdmin(this Customer customer, bool onlyActiveCustomerRoles = true)
        {
            bool ret = IsInCustomerRole(customer, SystemCustomerRoleNames.Administrators, onlyActiveCustomerRoles)
                || IsInCustomerRole(customer, SystemCustomerRoleNames.GlobalAdministrators, onlyActiveCustomerRoles);

            return ret;
        }

        public static bool IsStoreAdmin(this Customer customer, bool onlyActiveCustomerRoles = true)
        {
            bool ret = IsInCustomerRole(customer, SystemCustomerRoleNames.StoreAdministrators, onlyActiveCustomerRoles);

            return ret;
        }

        public static string IsLoggedAs(this Customer customer, bool onlyActiveCustomerRoles = true)
        {
            string ret = "ADMIN";

            if (!IsSystemGlobalAdmin(customer))
            {
                if (IsStoreAdmin(customer))
                {
                    ret = "STORE.ADMIN";
                }
                else if (IsVendor(customer))
                {
                    ret = "VENDOR";
                }
            }
        ...
        public static bool IsReadOnly(this Customer customer,bool onlyActiveCustomerRoles=true)
        {
