...
namespace Nop.Core.Domain.Orders
{
    ...
    public partial class Order : BaseEntity
    {
        ...
        public bool HasShippingOrderItems
        {
            get
            {
                ...
                {
                    if (!item.IsDeliveryPickUp && item.IsShipping || item.SelectedShippingMethodName == "Delivery")
                        HasShipping = true;
                }
                return HasShipping;
            }
        ...
        public bool ProcessRefundWithVertex
        {
            get
            {
                var originalStoreList = new List<int> { 11, 18, 21, 24, 28, 29, 43, 44, 48, 68, 70, 83, 88, 94, 100, 104, 107, 111, 119, 120, 128, 137, 146, 147, 150, 161, 183, 187, 193, 199, 205, 211, 220, 222, 240, 241, 243, 250, 262, 270, 288, 304, 318, 322, 335, 337, 338, 339, 349, 350, 351, 355, 358, 359, 360, 362, 363, 371, 373, 376, 380, 381, 387, 397, 405, 406 };

                if (originalStoreList.Contains(this.StoreId))
                { 
                    return true;
                }
                else
                {
                    if (this.CreatedOnUtc <= new DateTime(2020, 2, 1, 0, 0, 0))
                    {
                        return false;
                    }
                    else
        ...
        public bool HasPickupForSingleProduct
        {
            get
            {
                bool haspickupwithSingleProd = false;
                var itemsforInstorePickup = OrderItems.Where(x => x.SelectedShippingMethodName == "In-Store Pickup").ToList().Count;
                var MealPlanItems = OrderItems.Where(x => x.SelectedShippingMethodName == null).ToList().Count;
                if ((OrderItems.Count == itemsforInstorePickup) || MealPlanItems > 0)
                {
                    haspickupwithSingleProd = true;
                    return haspickupwithSingleProd;
                }
                else
                    return haspickupwithSingleProd;
            }
