...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            if ((storeId > 0 && !_catalogSettings.IgnoreStoreLimitations) || (!showHidden && !_catalogSettings.IgnoreAcl))
            {
                ...
                if (storeId > 0 && !_catalogSettings.IgnoreStoreLimitations)
                {
                    ...
                    query = from c in query
                            join pm in _productCategoryRepository.Table on c.Id equals pm.CategoryId
                            join p in _productRepository.Table on pm.ProductId equals p.Id
                            join sm in _storeMappingRepository.Table
                             on new { c1 = pm.ProductId, c2 = "Product" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into c_sm
                            from sm in c_sm.DefaultIfEmpty()
                            where currentStoreId == sm.StoreId && c.ShowOnHomePage && !c.Deleted && c.Published && p.Published && !p.Deleted && (!p.AvailableStartDateTimeUtc.HasValue || p.AvailableStartDateTimeUtc.Value < nowUtc) && (!p.AvailableEndDateTimeUtc.HasValue || p.AvailableEndDateTimeUtc.Value > nowUtc)
                            select c;

                    ////BELOW SECTION OF CODE HAS TTHE PARAMETER REMOVED  WITH P.PUBLISHED (WHICH MEANS IF A PRODUCT HAS SUBCATEGORIES AND IS NOT PUBLISHED WILL)
                    //query = from c in query
                    //        join pm in _productCategoryRepository.Table on c.Id equals pm.CategoryId
                    //        join p in _productRepository.Table on pm.ProductId equals p.Id
                    //        join sm in _storeMappingRepository.Table
                    //         on new { c1 = pm.ProductId, c2 = "Product" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into c_sm
                    //        from sm in c_sm.DefaultIfEmpty()
                    //        where currentStoreId == sm.StoreId && c.ShowOnHomePage && !c.Deleted && c.Published && (!p.AvailableStartDateTimeUtc.HasValue || p.AvailableStartDateTimeUtc.Value < nowUtc) && (!p.AvailableEndDateTimeUtc.HasValue || p.AvailableEndDateTimeUtc.Value > nowUtc)
                    //        select c;
                    #endregion
                }
                ...
                if (!showHidden && (!_catalogSettings.IgnoreAcl || !_catalogSettings.IgnoreStoreLimitations))
                {
                    ...
                    if (!_catalogSettings.IgnoreStoreLimitations)
                    {
                        ...
                        query = from c in query
                                ...
                                join sm2 in _storeMappingRepository.Table
                                on new { c1 = p.Id, c2 = "Product" } equals new { c1 = sm2.EntityId, c2 = sm2.EntityName } into c_sm2
                                from sm in c_sm.DefaultIfEmpty()
                                from sm2 in c_sm2.DefaultIfEmpty()
                                where !c.LimitedToStores || currentStoreId == sm.StoreId && p.Published
                                select c;
                        #endregion
                    }
                    //only distinct categories (group by ID)
        ...
        public virtual IList<Category> GetAllCategoriesDisplayedOnHomePage(bool showHidden = false)
        {
            ...
            //        where currentStoreId == sm.StoreId && c.ShowOnHomePage && !c.Deleted && c.Published && p.Published
            //        && (!p.AvailableStartDateTimeUtc.HasValue || p.AvailableStartDateTimeUtc.Value <= DateTime.UtcNow) && (!p.AvailableEndDateTimeUtc.HasValue || p.AvailableEndDateTimeUtc.Value >= DateTime.UtcNow)
            //        select c;

            //THE ABOVE METHOD TO BE USED IF WE WANT THE PRODUCTS THAT ARE NOT PUBLISHED FOR THEM THE CATEGORY WILL NOT SHOW.FOR THIS METHOD WE NEED TO DISABL THE CATEGORY IN THE STORE
            query = from c in query
                    join pm in _productCategoryRepository.Table on c.Id equals pm.CategoryId
                    join p in _productRepository.Table on pm.ProductId equals p.Id
                    join sm in _storeMappingRepository.Table
                    ...
                    from sm in c_sm.DefaultIfEmpty()
                    orderby c.DisplayOrder
                    where currentStoreId == sm.StoreId && c.ShowOnHomePage && !c.Deleted && c.Published && p.Published && !p.Deleted
                    && (!p.AvailableStartDateTimeUtc.HasValue || p.AvailableStartDateTimeUtc.Value <= DateTime.UtcNow) && (!p.AvailableEndDateTimeUtc.HasValue || p.AvailableEndDateTimeUtc.Value >= DateTime.UtcNow)
                    select c;
