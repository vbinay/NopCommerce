...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductTagService : IProductTagService
    {
			...
			int storeId = 0) /// NU-42
        {
            ...
            #region NU-42
            List<int> productTagsToRemove = new List<int>();

            foreach(var q in query)
            {
                foreach(var product in q.Products)
                {
                    var mapping = _storeMappingRepository.Table.Where(sm => sm.EntityId == product.Id && sm.StoreId == storeId);
                    if (mapping == null || product.IsMaster)
                    {
                        productTagsToRemove.Add(q.Id);
                    }
                }
            }
            #endregion
            ...
            #region NU-10
            foreach (var pt in productTagsToRemove)
            {
                productTags.RemoveAll((p) => p.Id == pt);
            }
            #endregion
