...
namespace Nop.Services.Catalog
{
    ...
    public partial class CopyProductService : ICopyProductService
    {
        ...
        /// <param name="isCopyMasterProduct">Should the Copied Product be a master product?</param>
        ...
        public virtual Product CopyProduct(Product product, string newName,
			...
			bool isCopyMasterProduct = true) /// NU-10
        {
            if (product == null)
            ...
            var productCopy = new Product
            {
                ...
                Sku = product.Sku,
                IsMaster = isCopyMasterProduct, /// NU-10
                MasterId = !isCopyMasterProduct ? product.Id : -1,  /// NU-10
                ManufacturerPartNumber = product.ManufacturerPartNumber,
                Gtin = product.Gtin,
                IsGiftCard = product.IsGiftCard,
                IsDonation = product.IsDonation, /// NU-17
                IsMealPlan = product.IsMealPlan, /// NU-16
                ...
                DisableBuyButtonForGuestUsers = product.DisableBuyButtonForGuestUsers,
                ...
                ProductionDaysLead = product.ProductionDaysLead, /// NU-18
                ProductionHoursLead = product.ProductionHoursLead, /// NU-18
                ProductionMinutesLead = product.ProductionMinutesLead, /// NU-18
                IsPickupEnabled = product.IsPickupEnabled, /// NU-13
                CheckoutNotes = product.CheckoutNotes, /// NU-19
                IsReservation = product.IsReservation ? product.IsReservation : false,
                ReservationStartTime = product.IsReservation ? product.ReservationStartTime : null,
                ReservationEndTime = product.IsReservation ? product.ReservationEndTime : null,                            
                ReservationInterval = product.IsReservation ? product.ReservationInterval : 0 ,
                MaxOccupancy = product.IsReservation ? product.MaxOccupancy : 0,
                ReservationProducts = product.IsReservation ? _productService.GetReservationProductsbyProductId(product.Id) : null,
                LeadTime = product.IsReservation ? product.LeadTime :0,
                MaxWindowDays = product.IsReservation ? product.MaxWindowDays : 0,
                ReservationCapPerSlot = product.IsReservation ? product.ReservationCapPerSlot : 0,
                IsBundleProduct = product.IsBundleProduct ? product.IsBundleProduct: false
            };
            ...
            //update GLCode of the productCopy
            foreach (var glcode in product.ProductGlCodes)
            {


                productCopy.ProductGlCodes.Add(new ProductGlCode
                {
                    Amount = glcode.Amount,
                    GlCode = glcode.GlCode,
                    GlCodeId = glcode.GlCodeId,
                    Percentage = glcode.Percentage,
                    Product = productCopy,
                    ProductId = productCopy.Id
            ...
            productCopy.ShowStandardMealPlanFields = product.ShowStandardMealPlanFields;  //rmarkle SWU-631
