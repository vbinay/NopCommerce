...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        private readonly IRepository<ProductWarehouse> _productWarehoueMapping;
        private readonly IRepository<Product> _productRepository;
        private readonly IRepository<StagingProducts> _StagingproductRepository;
        private readonly IRepository<ProductRelation> _productRelationRepository;
        ...
        private readonly IRepository<ReservationProducts> _reservationProductsRepo;
        private readonly IRepository<ReservedProduct> _reservedProductRepo;
        private readonly IRepository<ProductFulfillmentCalendar> _productFulfillmentCalendarRepo;
        #endregion
        ...
        /// <param name="StagingproductRepository">Staging product Repository</param>
        ...
        public ProductService(IRepository<ProductGlCodeCalculation> productGlcodecalcrepo, ICacheManager cacheManager,
            IRepository<Product> productRepository,
            IRepository<StagingProducts> StagingproductRepository,
            ...
            IRepository<ProductRelation> productRelationRepository,
            IRepository<ProductWarehouse> productWarehoueMapping,
            ...
            IRepository<ReservationProducts> reservationProductsRepo,
            IRepository<ReservedProduct> reservedProductRepo,
            IRepository<ProductFulfillmentCalendar> productFulfillmentCalendarRepo)
        {
            this._productGlcodecalcrepo = productGlcodecalcrepo;
            this._cacheManager = cacheManager;
            this._productRepository = productRepository;
            this._StagingproductRepository = StagingproductRepository;
            ...
            this._productWarehoueMapping = productWarehoueMapping;
            ...
            this._reservationProductsRepo = reservationProductsRepo;
            this._reservedProductRepo = reservedProductRepo;
            this._productFulfillmentCalendarRepo = productFulfillmentCalendarRepo;
        }
        ...
        public virtual IList<ProductFulfillmentCalendar> GetUnavlblDates(int productId, DateTime fromDate, DateTime toDate)
        {
            var calDays = from c in _productFulfillmentCalendarRepo.Table
                          where
        ...
        public virtual IList<ReservationProducts> GetReservationProductsbyProductId(int productId)
        {
            var query = from p in _reservationProductsRepo.Table
                        orderby p.Id
                        where p.ProductId == productId
                        select p;
            var reservationProducts = query.ToList();
            return reservationProducts ;
        }

        public virtual IList<ReservedProduct> GetReservedProductbyProductId(int productId)
        {
            var query = from p in _reservedProductRepo.Table
                        orderby p.Id
                        where p.ProductId == productId
                        select p;
            var reservedProducts = query.ToList();
        ...
        public virtual IList<ProductFulfillmentCalendar> GetProductFulfillmentCalendarbyProductId(int productId)
        {
            var query = from p in _productFulfillmentCalendarRepo.Table
                        orderby p.Id
                        where p.ProductId == productId
                        select p;
            var productFulfillmentCalendar = query.ToList();
            return productFulfillmentCalendar;
        }
        ...
        public virtual void DeleteProduct(Product product)
        {
         ...
         public virtual void DeleteReservationProducts(ReservationProducts reservationProduct)
        {
            if (reservationProduct == null)
                throw new ArgumentNullException("reservationProduct");

            _reservationProductsRepo.Delete(reservationProduct);
        }
        ...
        /// <summary>
        /// Product Warehosue mapping
        /// </summary>
        /// <param name="productId"></param>
        /// <returns></returns>
        public virtual IList<ProductWarehouse> GetPickupWarehouseByProductId(int productId)
        {

            var query = from wm in _productWarehoueMapping.Table
        ...
        public virtual void UpdateProductPickupWarehouse(int productID, int warehouseID)
        {
            var model = new ProductWarehouse
            {
                ProductId = productID,
                WarehouseId = warehouseID,
            };
            _productWarehoueMapping.Insert(model);

        }

        public virtual void DeleteProductPickupWarehouse(ProductWarehouse warehouseData)

        {
            _productWarehoueMapping.Delete(warehouseData);
        }
        ...
        public virtual Product GetProductById_Cache(int productId)
        {
            if (productId == 0)
                return null;
            string key = string.Format(PRODUCTS_BY_ID_KEY, productId);
        ...
        public virtual void InsertReservationProducts(ReservationProducts reservationProducts)
        {
            if (reservationProducts == null)
                throw new ArgumentNullException("Reservation Product object is Null");

            //insert
            _reservationProductsRepo.Insert(reservationProducts);

            //event notification
            _eventPublisher.EntityInserted(reservationProducts);
        }
        ...
        public virtual int GetNumberOfProductsInCategory(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            //THE METHOD TO BE USED IF WE WANT THE PRODUCTS THAT ARE NOT PUBLISHED FOR THEM THE CATEGORY WILL NOT SHOW.FOR THIS METHOD WE NEED TO DISABL THE CATEGORY IN THE STORE
            query = query.Where(p => !p.Deleted && p.Published && p.VisibleIndividually);
            //CANNOT USE ANY OF THE PROCESS AS THIS IS CHECKING FOR THE MAPPING INFORMATION AND IN ORER TO MAINTAIN THE PARENT SUBCATERGY DATA IN DISPLAY WITHOUT HAVING TO PUBLISH ANYTHING IN THE PARENT CATEGORY WE HAVE TO ELIMINTAE THE MAPPING INFORMATION
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            int masterProductId = 0, /// NU-10
            bool? overrideHideMasterProducts = null) /// NU-10
        {
            ...
            return SearchProducts(out filterableSpecificationAttributeOptionIds, false,
                ...
                masterProductId, /// NU-10
                overrideHideMasterProducts); /// NU-10
        }
            ...
            bool? overrideHideMasterProducts = null) /// NU-10
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                #region NU-10
                var pMasterProductId = _dataProvider.GetParameter();
                pMasterProductId.ParameterName = "MasterProductId";
                pMasterProductId.Value = masterProductId;
                pMasterProductId.DbType = DbType.Int32;
                #endregion
                ...
                #region NU-10
                var pOverrideHideMasterProducts = _dataProvider.GetParameter();
                pOverrideHideMasterProducts.ParameterName = "OverrideHideMasterProducts";
                pOverrideHideMasterProducts.Value = overrideHideMasterProducts != null ? (object)overrideHideMasterProducts.Value : DBNull.Value;
                pOverrideHideMasterProducts.DbType = DbType.Boolean;
                #endregion
                ...
                var products = _dbContext.ExecuteStoredProcedureList<Product>(
                    ...
                    pShowHidden,
                    pOverridePublished,
                    pOverrideHideMasterProducts,// NU-10
            ...
            int storeId = 0) /// NU-42
        {
            ...
            var query = from p in _productRepository.Table
                        ...
                        p.MinStockQuantity >= (
                           ...
                           p.StockQuantity) &&
                        (vendorId == 0 || p.VendorId == 0)
                        select p;

            #region NU-42
            if (vendorId == 0 && storeId > 0)
            {
                query = from q in query
                        join sm in _storeMappingRepository.Table
                            on q.Id equals sm.EntityId
                        orderby q.MinStockQuantity
            ...
            int storeId = 0) /// NU-42
        {
            ...
            if (vendorId == 0 && storeId > 0)
            {
                query = from q in query
                        join sm in _storeMappingRepository.Table
                            on q.ProductId equals sm.EntityId
                        where sm.EntityName == "Product"
                           && sm.StoreId == storeId
                        select q;
            }
        ...
        public void InsertJeGlCodeAndAmountBusinessRules(ProductGlCodeMapping itemsMapping)
        {
            if (itemsMapping == null)
                throw new ArgumentException("itemsMapping");

            _productGlcodeMappingRepo.Insert(itemsMapping);
            _productGlcodeMappingRepo.Detach(itemsMapping);
        ...
        #region NU-10
        public virtual IList<int> GetProductChildren(Product product)
        {
            var productChildren = from p in _productRepository.Table
                                  where p.MasterId == product.Id
                                    && p.IsMaster == false
                                    && p.Deleted == false
                                  select p;

            return productChildren.Select(p => p.Id).ToList();
        }

        public virtual Product GetProductMaster(Product product)
        {
            Product ret = null;

            if (!product.IsMaster)
            {
                ret = GetProductById(product.MasterId);
            }
        ...
        public IPagedList<ProductRelation> GetProductRelationsByStore(int pageIndex = 0, int pageSize = int.MaxValue,
            int storeId = 0)
        {
            var query =
             from p in _productRelationRepository.Table
             orderby p.Id
             where p.StoreId == storeId
             select p;


            return new PagedList<ProductRelation>(query, pageIndex, pageSize);

        }

        public IPagedList<ProductRelation> GetProductRelationsByProduct(Product product, int pageSize = int.MaxValue, int pageIndex = 0)
        {
            var query =
           from p in _productRelationRepository.Table
           orderby p.Id
        ...
        public IList<ProductRelation> GetProductRelationsByStore(int storeId, int typeId)
        {
            var query =
             from p in _productRelationRepository.Table
             orderby p.Id
             where p.StoreId == storeId && p.RelationTypeId == typeId
             select p;


            return query.ToList<ProductRelation>();

        }

        public IList<ProductRelation> GetProductRelationsByProduct(int productId, int typeId)
        {
            var query =
           from p in _productRelationRepository.Table
           orderby p.Id
           where p.ProductId == productId && p.RelationTypeId == typeId
           select p;
            return query.ToList<ProductRelation>();
        }

        public IList<ProductRelation> GetProductRelationsByRelatedProduct(int RelatedProductId, int typeId)
        {
            var query =
           from p in _productRelationRepository.Table
           orderby p.Id
           where p.RelatedProductId == RelatedProductId && p.RelationTypeId == typeId
           select p;
            return query.ToList<ProductRelation>();
        }
        ...
        public void InsertProductRelation(RelatedProduct relatedProduct)
        {
            throw new NotImplementedException();
        }

        public void UpdateProductRelation(RelatedProduct relatedProduct)
        {
            throw new NotImplementedException();
        }

        public void InsertStagingProductInfo(StagingProducts StagingProducts)
        {
            if (StagingProducts == null)
                throw new ArgumentNullException("StagingProducts");

            _StagingproductRepository.Insert(StagingProducts);

            _eventPublisher.EntityInserted(StagingProducts);
        }


        public bool StagingProductExists(int productId)
        {
            var query =
            from p in _StagingproductRepository.Table
            orderby p.Id
            where p.ProductId == productId
            select p;
        ...
        public StagingProducts GetStagingMapping(int productId)
        {
            var query =
             from p in _StagingproductRepository.Table
             orderby p.Id
            ...
            {
                return new StagingProducts();

            }

        }

        public void UpdateStagingMapping(StagingProducts stagingProducts)
        {
            if (stagingProducts == null)
                throw new ArgumentNullException("StagingProducts");

            string SqlQuery = @"Update [StagingTaxCategory] set  
                   TaxCategoryId= @TaxCategoryId
                  , GLCode1Id= @GlCode1Id
                  , GlCode1Amount= @GlCode1Amount
                  , GlCode2Id= @GlCode2Id
                  , GLCode2Amount= @GLCode2Amount
                  , Comments= @Comments
                  , Completed= @Completed
                  , GlCode3Id= @GlCode3Id
                  , GlCode3Amount= @GlCode3Amount
                  , MasterProductId= @MasterProductId
                  where id = " + stagingProducts.Id.ToString();
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                cmd.Parameters.Add("@TaxCategoryId", stagingProducts.TaxCategoryId);
                cmd.Parameters.Add("@GlCode1Id", stagingProducts.GlCode1Id);
                cmd.Parameters.Add("@GlCode1Amount", stagingProducts.GlCode1Amount);
                cmd.Parameters.Add("@GlCode2Id", stagingProducts.GlCode2Id);
                cmd.Parameters.Add("@GLCode2Amount", stagingProducts.GLCode2Amount);
                cmd.Parameters.Add("@GlCode3Id", stagingProducts.GlCode3Id);
                cmd.Parameters.Add("@GlCode3Amount", stagingProducts.GLCode3Amount);
                cmd.Parameters.Add("@MasterProductId", stagingProducts.MasterProductId);
                cmd.Parameters.Add("@Comments", stagingProducts.Comments == null ? "" : stagingProducts.Comments);
                cmd.Parameters.Add("@Completed", stagingProducts.Completed);
            ...
            //update
            _StagingproductRepository.Update(stagingProducts);

            //event notification
            _eventPublisher.EntityUpdated(stagingProducts);
        }
    }
}
