...
namespace Nop.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        protected virtual string ProductListToHtmlTable(Order order, int languageId, int vendorId)
        {
            ...
            #region NU-13
            sb.AppendLine(string.Format("<th>{0}</th>", _localizationService.GetResource("Delivery Method", languageId)));
            sb.AppendLine(string.Format("<th>{0}</th>", _localizationService.GetResource("Requested Pickup/Delivery Date", languageId)));
            sb.AppendLine(string.Format("<th>{0}</th>", _localizationService.GetResource("Pickup Location", languageId)));
            #endregion
            ...
            var table = order.OrderItems.ToList(); //_orderService.GetOrderItemsByOrderID(order.Id).ToList();
            ...
            for (int i = 0; i <= table.Count - 1; i++)
            {
                ...
                if (orderItem.IsBundleProduct)
                {
                    ...
                    //orderItem.Product = parentBundleProduct ;

                    if (i > 0 && table[0].Order.Id == table[i].Order.Id)
                    {
                        List<ShoppingCartBundleProductItem> scbp = _shoppingcartService.GetAssociatedProductsByOrderItemId(table[0].Id).ToList();
                ...
                var product = orderItem.IsBundleProduct ? parentBundleProduct : orderItem.Product;
                if (product == null)
                    continue;
                ...
                if (orderItem.IsReservation)
                {
                    ...
                    reservedProduct = _orderService.GetReservedProductbyOrderItemId(orderItem.Id);
                    sb.AppendLine("<br />");
                    sb.AppendLine("Reservation Date : " + reservedProduct.ReservationDate.ToShortDateString());
                    sb.AppendLine("<br />");
                    sb.AppendLine("Reservation Time : " + reservedProduct.ReservedTimeSlot);
                }
                ...
                #region NU-13
                if (orderItem.SelectedShippingMethodName == "Contract Shipping")
                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", orderItem.SelectedShippingMethodName + " : " + orderItem.RCNumber));
                else if (orderItem.SelectedShippingMethodName == "MailStop Shipping")
                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", orderItem.SelectedShippingMethodName + " : " + orderItem.MailStopAddress));
                if (orderItem.SelectedShippingMethodName == "In-Store Pickup")
                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", orderItem.SelectedShippingMethodName));
                

                if (orderItem.RequestedFulfillmentDateTime != null)
                {

                    string format = "";

                    if (orderItem.SelectedShippingMethodName == "Delivery")
                    {

                        if (orderItem.SelectedFulfillmentWarehouse != null)
                        {
                            format = orderItem.SelectedFulfillmentWarehouseId != null ? orderItem.SelectedFulfillmentWarehouse.DeliveryDateFormat : "d";
                        }
                    }
                    else if (orderItem.SelectedShippingMethodName == "In-Store Pickup")
                    {
                        if (orderItem.SelectedFulfillmentWarehouse != null)
                        {
                            format = orderItem.SelectedFulfillmentWarehouseId != null ? orderItem.SelectedFulfillmentWarehouse.PickupDateTimeFormat : "d";
                        }

                    }

                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", orderItem.RequestedFulfillmentDateTime != null ? String.Format("{0:" + format + "}", orderItem.RequestedFulfillmentDateTime) : ""));
                }
                else
                {
                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", ""));
                }
                ...
                if (!string.IsNullOrEmpty(orderItem.SelectedShippingMethodName) && orderItem.SelectedShippingMethodName == "In-Store Pickup")
                {
                    ...
                    if (orderItem.SelectedFulfillmentWarehouse != null)
                    {
                        sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", orderItem.SelectedFulfillmentWarehouse != null ? orderItem.SelectedFulfillmentWarehouse.Name + "<br/>" : "")); //TODO:  Need the warehouse address
                    }
                }
                else
                {
                    sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", "N/A"));
                    if (orderItem.SelectedFulfillmentWarehouse == null)
                    {
                        sb.AppendLine(string.Format("<td style=\"padding: 0.6em 0.4em;text-align: right;\">{0}</td>", "N/A"));
                    }
            ...
            AddStoreTokens(IList<Token> tokens, Store store, EmailAccount emailAccount)
        {
            ...
            #region NU-68
            var defaultEmailAccount = _storeService.GetStoreContactByEmailTypeId(1, store.Id);

            string[] emails = defaultEmailAccount.Email.Split(';');

            StringBuilder emailString = new StringBuilder();

            for (int i = 0; i < emails.Length; i++)
            {
                if (!emails[i].Contains("@hpeprint.com"))
                {
                    emailString.Append(emails[i] + (i < emails.Length ? ";" : ""));
                }
            }
            if (!string.IsNullOrEmpty(emailString.ToString()))
            {
                tokens.Add(new Token("Store.Email", emailString.ToString()));
            }
            #endregion
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            tokens.Add(new Token("Order.ShippingMethod", order.ShippingMethod));//Commented out SODMYWAY-3467
        ...
        public virtual void AddReturnRequestTokens(IList<Token> tokens, ReturnRequest returnRequest, OrderItem orderItem)
        {
            ...
            tokens.Add(new Token("Order.Product(s)", ProductListToHtmlTable(orderItem.Order, 1, 0), true));
            tokens.Add(new Token("ReturnRequest.ID", returnRequest.Id.ToString()));
            tokens.Add(new Token("ReturnRequest.OrderSubtotalExltax", orderItem.Order.OrderSubtotalExclTax.ToString()));
            tokens.Add(new Token("ReturnRequest.OrderTax", orderItem.Order.OrderTax.ToString()));
            tokens.Add(new Token("ReturnRequest.OrderTotal", orderItem.Order.OrderTotal.ToString()));
        ...
        public virtual void AddProductTokens(IList<Token> tokens, Product product, int languageId)
        {
            ...
            tokens.Add(new Token("Product.CheckoutNote", product.CheckoutNotes));   /// NU-19
        ...
        #region NU-69
        public virtual void AddDonationTokens(IList<Token> tokens, Order order, List<Donation> donations, int languageId, bool includeGiftAmount = false)
        {
            tokens.Add(new Token("Donation.DonorFirstName", donations[0].DonorFirstName));
            tokens.Add(new Token("Donation.DonorLastName", donations[0].DonorLastName));
            tokens.Add(new Token("Donation.DonorCompany", donations[0].DonorCompany));
            tokens.Add(new Token("Donation.OnBehalfOfFullName", donations[0].OnBehalfOfFullName));
            IEnumerator<OrderItem> en = order.OrderItems.GetEnumerator();
            en.MoveNext();


            tokens.Add(new Token("Product.ShortDescription", en.Current.Product.ShortDescription));
            tokens.Add(new Token("Product.CheckoutNotes", en.Current.Product.CheckoutNotes));   /// NU-19

            // Language language = _languageService.GetLanguageById(languageId);


            bool includeGift = false;
            decimal total = 0;

            foreach (Donation donation in donations)
            {
                if (donation.IncludeGiftAmount)
                {
                    includeGift = true;
                }
            ...
            decimal opvPriceExclTaxInCustomerCurrency = _currencyService.ConvertCurrency(total, order.CurrencyRate);
            string priceStr = _priceFormatter.FormatPrice(opvPriceExclTaxInCustomerCurrency, true, false);

            tokens.Add(new Token("Donation.Amount", priceStr));

            if (includeGift)
            {
                tokens.Add(new Token("Donation.AmountText", "for the amount of " + priceStr));
                tokens.Add(new Token("Donation.GiftAmountFullText", "The donation was in the amount of " + priceStr));
            }
            else
            {
                tokens.Add(new Token("Donation.AmountText", ""));
                tokens.Add(new Token("Donation.GiftAmountFullText", "The donation was in the amount of " + priceStr));
            }
        ...
        #region NU-84
        public virtual void AddWarehouseTokens(IList<Token> tokens, int warehouseID, EmailAccount emailAccount)
        {
            if (emailAccount == null)
                throw new ArgumentNullException("emailAccount");

            var warehouse = _warehouseRepository.Table.Where(w => w.Id == warehouseID).FirstOrDefault();
            var store = _storeService.GetStoreById(_storeMappingService.GetStoreMappings(warehouse).FirstOrDefault().StoreId);
            var address = _addressService.GetAddressById(warehouse.AddressId);
            string CompanyAddress = address.Address1 + address.Address2 + @"&#10;" + address.City + ", " + address.StateProvince.Abbreviation + @"&#10;" + address.ZipPostalCode;

            tokens.Add(new Token("Store.Name", warehouse.Name));
            tokens.Add(new Token("Store.URL", store.Url, true));

            #region NU-68
            var defaultEmailAccount = _storeService.GetAllStoreContacts(store.Id).FirstOrDefault();

            string[] emails = emailAccount.Email.Split(';');
            ...
            tokens.Add(new Token("Store.CompanyName", warehouse.Name));
            tokens.Add(new Token("Store.CompanyAddress", CompanyAddress));
            tokens.Add(new Token("Store.CompanyPhoneNumber", address.PhoneNumber));
            tokens.Add(new Token("Store.CompanyVat", ""));
