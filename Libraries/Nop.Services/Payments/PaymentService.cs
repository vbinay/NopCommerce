...
namespace Nop.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        {
            bool isTroveEnabledStore = false;
            ...
            if (troveStoreIds != "")
            {
                ...
                {
                    if (storeIdString == refundPaymentRequest.Order.StoreId.ToString())
                    {
                        isTroveEnabledStore = true;
                    }
                }
            }
            ...
            else
            {
                ...
                try
                {
                    string realId = refundPaymentRequest.Order.BillingAddress.RealId;
                    int biteOrderId = refundPaymentRequest.Order.Id;
                    float amount = (float)refundPaymentRequest.Order.OrderTotal;
                    ...
                    TroveWalletsTokenResponseModel refundToken = TroveUtilities.GetWalletsToken(realId);
                    string refundTokenStr = refundToken.Token;

                    PostPaymentToWalletRequest body2 = new PostPaymentToWalletRequest
                    {
                        mti = "refund",
                        pcode = TroveUtilities.CreatePcode(pCode),
                        amount = amount,
                        stan = biteOrderId,
                        date = TroveUtilities.CreateDateString(date),
                        rrn = refundPaymentRequest.Order.SubscriptionTransactionId,   //ccPaymentResponse.rrn,// "000000209F92","00000020A5E1",/
                        tid = terminalId,
                        mid = merchantId,
                        mealPlan = TroveUtilities.CreateFlag(mealPlan),
                        token = refundTokenStr,
                    };
                    PostPaymentToWalletResponse ccRefundPaymentResponse = TroveUtilities.PostPaymentToWallet(body2);
                    if(ccRefundPaymentResponse.Success)
                    {
