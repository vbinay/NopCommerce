    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            out SortedDictionary<decimal, decimal> taxRates)
        {
            ...
            foreach (var shoppingCartItem in cart)
            {
                ...
                decimal taxRate;
                decimal sciExclTax = _taxService.GetProductPrice(shoppingCartItem.Product, sciSubTotal, false, customer,false, out taxRate);
                decimal sciInclTax = _taxService.GetProductPrice(shoppingCartItem.Product, sciSubTotal, true, customer,false, out taxRate);
            ...
            out decimal taxRate, out List<Discount> appliedDiscounts)
        {
            ...
            bool IsTieredShippingEnabled = _storeContext.CurrentStore.IsTieredShippingEnabled == true ? true : false;
            var customer = cart.GetCustomer();
            ...
            List<KeyValuePair<DateTime, KeyValuePair<ShippingOption, List<ShoppingCartItem>>>> shippingBuckets = GetShippingBuckets(cart, customer);
            Dictionary<string, decimal> ShippingBucketsCollection = new Dictionary<string, decimal>();

            decimal? rate = 0;
            int count = 0;

            if (!IsTieredShippingEnabled)
            {
                foreach (var bucket in shippingBuckets)
                {
                    if (bucket.Value.Key.Name == "Delivery")
                    {
                        //grab it from the selected warehouse

                        rate += bucket.Value.Value[0].SelectedWarehouse.DeliveryFee;
                        ShippingBucketsCollection.Add("Delivery-" + count + bucket.Value.Key.ShoppingCartProductId + "", bucket.Value.Value[0].SelectedWarehouse.DeliveryFee);
                    }
                    else if (bucket.Value.Key.Name == "In-Store Pickup")
                    {
                        //grab it from the selected warehouse

                        rate += bucket.Value.Value[0].SelectedWarehouse.PickupFee;
                        ShippingBucketsCollection.Add("In-Store Pickup-" + count + bucket.Value.Key.ShoppingCartProductId + "", bucket.Value.Value[0].SelectedWarehouse.PickupFee);

                    }
                    else if (bucket.Value.Key.Name.Contains("UPS"))
                    {
                        foreach (var test in bucket.Value.Value)
                        {

                            rate += bucket.Value.Key.Rate;
                            // ShippingBucketsCollection.Add("UPS-" + bucket.Value.Key.ShoppingCartProductId + "", bucket.Value.Key.Rate);
                            ShippingBucketsCollection.Add("UPS-" + count + test.ProductId + "", bucket.Value.Key.Rate);
                    ...
                    {
                        //grab it from the shippingOption
                        rate = rate.GetValueOrDefault(decimal.Zero) + (bucket.Value.Key != null ? bucket.Value.Key.Rate : 0);
                    }
            ...
            shippingTotal = AdjustShippingRate(rate.GetValueOrDefault(decimal.Zero), cart, out appliedDiscounts);
            _httpContext.Session["shippingBucket"] = ShippingBucketsCollection;

            if (shippingBuckets.Count == 0)
            {
            ...
            if (shippingTotal.HasValue)
            {
                ...
                try
                {
                    if (IsTieredShippingEnabled && includingTax)
                    {
                        if (HttpContext.Current.Session["VertexCurrentSessionId"] != null)
                        {
                            string vertexSessionId = HttpContext.Current.Session["VertexCurrentSessionId"].ToString();
                            var firstProduct = _workContext.CurrentCustomer.ShoppingCartItems.Where(x => x.IsFirstCartItem && x.FlatShipping > 0).FirstOrDefault();
                            if (firstProduct != null)
                            {
                                var item = _taxService.GetTaxRateBySessionIdProductIdTaxCode(firstProduct.Product.Id, "SHIP", vertexSessionId);
                                shippingTotalTaxed = shippingTotal + item.Tax;
                            }
                        }
                    }
                }
                ...
                {
                    throw new NopException("Shipping rate computation method could not be loaded",ex);
                }
        ...
        public List<KeyValuePair<DateTime, KeyValuePair<ShippingOption, List<ShoppingCartItem>>>> GetShippingBuckets(IList<ShoppingCartItem> cart, Customer customer)
        {
            List<KeyValuePair<DateTime, KeyValuePair<ShippingOption, List<ShoppingCartItem>>>> shippingBuckets = new List<KeyValuePair<DateTime, KeyValuePair<ShippingOption, List<ShoppingCartItem>>>>();
            List<ShippingOption> shippingOptions = new List<ShippingOption>();
            foreach (var item in cart)
            {
                ShippingOption lastShippingOption = null;

                lastShippingOption = customer.GetAttribute<ShippingOption>(SystemCustomerAttributeNames.SelectedShippingOption + "" + item.Id, _storeContext.CurrentStore.Id);
                if(lastShippingOption != null)
                    shippingOptions.Add(lastShippingOption);

                if (lastShippingOption != null)
                {
                    lastShippingOption.ShoppingCartProductId = item.Product.Id;

                    DateTime fulfillmentDate = item.RequestedFullfilmentDateTime.GetValueOrDefault();
                    ...
                    if (!found)
                    {
                        List<ShoppingCartItem> listOfShoppingCartItems = new List<ShoppingCartItem>();
                        listOfShoppingCartItems.Add(item);

                        shippingBuckets.Add(
                            new KeyValuePair<DateTime, KeyValuePair<ShippingOption, List<ShoppingCartItem>>>(
                                fulfillmentDate,
                                new KeyValuePair<ShippingOption, List<ShoppingCartItem>>(lastShippingOption,
                                                                                         listOfShoppingCartItems)));
                    }
                }
            }
            ...
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            if (_storeContext.CurrentStore.IsTieredShippingEnabled == true)
            {
                ...
                {
                    shoppingCartShipping += sci.FlatShipping;
                }
            }
            else
            {
                shoppingCartShipping = GetShoppingCartShippingTotal(cart, false);
            }
