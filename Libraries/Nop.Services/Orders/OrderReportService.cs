...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
            ...
            IRepository<GlCode> glCodeRepository)	
        {
            ...
            this._glCodeRepository = glCodeRepository;
        }
        ...
        public virtual OrderAverageReportLine GetOrderAverageReportLine(int storeId = 0,
            ...
            bool onlyVendorOrders = false)	/// SODMYWAY-
        {
            #region NU-42
            ...
            var item = (from oq in query
						...
						select new
						           {
                                       ...
                                       OrderTotalSum = result.Sum(o => o.OrderTotal - o.RefundedAmount),	/// SODMYWAY-
                                   }
					   ).Select(r => new OrderAverageReportLine
                       {
                           ...
                           #region SODMYWAY-
                           SumCommission = decimal.Zero,
                           SumEarned = decimal.Zero,
                           SumNoTax = decimal.Zero
                           #endregion
                       })
                       .FirstOrDefault();

            #region SODMYWAY-
            if (item != null && onlyVendorOrders)
            {
                foreach (var order in query)
                {
                    var orderItems = order.OrderItems.Where(oi => oi.Product.VendorId > 0);
                    if(vendorId > 0)
                    {
                        orderItems = order.OrderItems.Where(oi => oi.Product.VendorId == vendorId);
                    }

                    foreach (var orderItem in orderItems)
                    {
                        var commission = orderItem.StoreCommission != null ? orderItem.StoreCommission : 0;

                        item.SumCommission += (decimal)(orderItem.PriceExclTax * (commission / 100));
                        item.SumNoTax += (decimal)orderItem.PriceExclTax;
                    }
                }

                item.SumEarned = item.SumNoTax - item.SumCommission;
            }
            #endregion
            ...
            int storeId = 0)	/// NU-42
        {
            ...
            if (storeId > 0)
            {
                query1 = (from orderItem in _orderItemRepository.Table
                          join o in _orderRepository.Table on orderItem.OrderId equals o.Id
                          where (!createdFromUtc.HasValue || createdFromUtc.Value <= o.CreatedOnUtc) &&
                                (!createdToUtc.HasValue || createdToUtc.Value >= o.CreatedOnUtc) &&
                                (!o.Deleted) && (o.StoreId == storeId)
                          select orderItem.ProductId).Distinct();
            }
			#endregion
            ...
            string billingEmail = null, string billingLastName = "", string orderNotes = null)
        {
            ...
            var profit = reportSummary.SumOrders - reportSummary.SumTax;
            return profit;
        }

		#region NU-71
        public virtual IPagedList<OrderByDeliveryReportLine> SearchDeliveriesByUnit(int storeId, DateTime? startDate, DateTime? endDate, string unitNumber, TimeZoneInfo timeZone, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            startDate = startDate.HasValue ? startDate.Value.Date : DateTime.Today.Date;
            endDate = endDate.HasValue ? endDate.Value : DateTime.Today.AddDays(1).AddTicks(-1);//11:59:59 of current day

            var ordersQRY = _orderRepository.Table.Where(o => !o.Deleted);
            var ordersItemsQRY = (from o in ordersQRY 
                                  join oi in this._orderItemRepository.Table on o.Id equals oi.OrderId
                                  where startDate.Value <= oi.FulfillmentDateTime && endDate.Value >= oi.FulfillmentDateTime && o.OrderStatusId == 30
                                  select new
                                  {
                                      OrderId = o.Id,
                                      OrderGuid = o.OrderGuid,
                                      OrdItmProduct = oi.Product,
                                      StoreId = o.StoreId,
                                      CreatedOnUtc = o.CreatedOnUtc,
                                      OrdItmDeliveryDate = oi.FulfillmentDateTime,
                                      OrdItmExclTax = oi.PriceExclTax,
                                      OrdItmTax = oi.PriceInclTax - oi.PriceExclTax,
                                      OrdItmInclTax = oi.PriceInclTax
                                  });
            //get orders already delivered within the start and end date, or orders scheduled for delivery (future deliveries) within the start and end dates provided
            //query1 = query1.Where(o => (startTime.Value <= o.DeliveryDateUtc && endTime.Value >= o.DeliveryDateUtc && o.OrderStatusId == 30));
            //ordersQRY = ordersQRY.OrderByDescending(o => o.CreatedOnUtc);

            var reportQRY = (from o in ordersItemsQRY
                             join s in this._storeRepository.Table on o.StoreId equals s.Id
                             join pc in this._productCategoryMappingRepository.Table on o.OrdItmProduct.Id equals pc.ProductId                             
                             join pg in this._productGlCodeRepository.Table on pc.ProductId equals pg.ProductId
                             join p in this._productRepository.Table on pg.ProductId equals p.Id
                             join gl in this._glCodeRepository.Table on pg.GlCodeId equals gl.Id
                             where p.Deleted == false
                             where p.IsMealPlan == false
                           //  where gl.IsDelivered 
                         select new OrderByDeliveryReportLine()
                         {
                             Unit = s.ExtKey,                    
                             GLCode = gl.GlCodeName,
                             OrderId = o.OrderId,
                             OrdItmDeliveryDate = o.OrdItmDeliveryDate,
                             OrderSubTotalExclTax = o.OrdItmExclTax,
                             OrderTax = o.OrdItmTax,                  
                             OrderSubtotalInclTax = o.OrdItmInclTax,
                             ExtKey = s.ExtKey
                         }
                         ).Distinct();
            if (!String.IsNullOrEmpty(unitNumber))
                reportQRY = reportQRY.Where(s => s.ExtKey == unitNumber);

            reportQRY = reportQRY.OrderBy(x => x.ExtKey);

            var orders = reportQRY.ToList();

            return new PagedList<OrderByDeliveryReportLine>(orders, pageIndex, pageSize);
        }

        public virtual IList<GlCode> GetGlCodesForReport(bool isDelivered)
        {
            var query = from c in _glCodeRepository.Table
