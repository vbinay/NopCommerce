...
namespace Nop.Services.Orders
{
    ...
    public static class ShoppingCartExtensions
    {
        ...
        public static bool RequiresShipping(this IList<ShoppingCartItem> shoppingCart)
        {
            ...
            {
                if (shoppingCart[i].Product != null)
                {
                    if (!shoppingCart[i].Product.IsDonation && !shoppingCart[i].Product.IsMealPlan) /// NU-17 & NU-16
                    {
                        if (shoppingCart[i].Product.IsShipEnabled || shoppingCart[i].Product.IsPickupEnabled)
                        {
                            return true;
                        }
                    }
