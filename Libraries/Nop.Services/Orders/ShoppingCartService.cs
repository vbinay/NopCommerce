...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        private readonly IRepository<ShoppingCartBundleProductItem> _scbpiRepository;
        ...
        public ShoppingCartService(IRepository<ShoppingCartItem> sciRepository,
            IRepository<ShoppingCartBundleProductItem> scbpiRepository,
            ...
            IDateTimeHelper dateTimeHelper)
        {
            ...
            this._scbpiRepository = scbpiRepository;
            ...
            int storeId, bool automaticallyAddRequiredProductsIfEnabled)
        {
            ...
            if (product.RequireOtherProducts)
            {
                ...
                {
                    if (product.RequireOtherProductsOption == "OR")
                    {

                        var requiredProductids = product.RequiredProductIds;
                        if (cart.Where(x => x.ProductId == rp.Id).Any())
                        {
                            isfound = true;
                            warnings.Clear();
                            break;
                    ...
                    if (product.RequireOtherProductsOption == "AND")
                    {
                ...
                if (product.RequireOtherProductsOption=="OR" && !isfound)
                    warnings.Add(string.Format("In order to add this product, one of the following items must first be in the cart: {0}", products));
            }
        ...
        public virtual IList<string> GetStandardWarnings(Customer customer, ShoppingCartType shoppingCartType,
            ...
            int quantity,bool isParentBundleProduct = true)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            // Reservation Quantity Warning
            if (product.IsReservation && product.ReservationCapPerSlot > 0 && quantity > product.ReservationCapPerSlot)
            {
                warnings.Add("Max entered quantity should not be greater than: Reservation Cap Per Slot: " + product.ReservationCapPerSlot.ToString());
            }
                        ...
                        if (a1.Id == a2.Id)
                        {
                            ...
                            foreach (string str1 in attributeValuesStr)
                            {
                                if (!String.IsNullOrEmpty(str1.Trim()))
                                {
                                    ...
                                    //NU-90 Forcing validation because jquery isn't working
                                    if (a1.ProductAttribute.Name == "FSUCard #")
                                    {
                                        Match match = Regex.Match(str1.Trim(), @"\d{16}");
                                        if (!match.Success)
                                        {
                                            var notFoundWarning = a1.ProductAttribute.GetLocalized(a => a.Name) + ":  Please enter 16 digit card number"; //TODO: HARD CODED
                                            warnings.Add(notFoundWarning);
                                            warnings.Add("FieldError:product_attribute_" + a2.Id);
                                        }
                                    }
            ...
            foreach (var pam in attributes2)
            {
                ...
                if (pam.ValidationMinLength.HasValue)
                {
                        ...
                        pam.AttributeControlType == AttributeControlType.MultilineTextbox)
                    {
                        ...
                        if (pam.ValidationMinLength.Value > enteredTextLength)
                        {
                            ...
                            warnings.Add("FieldError:product_attribute_" + pam.Id);
                        }
                    }
                    if (pam.AttributeControlType == AttributeControlType.Checkboxes)
                    {
                        ...
                        if (valuesStr.Count < pam.ValidationMinLength.Value)
                        {
                            warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.CheckboxMinimumLength"), pam.ProductAttribute.GetLocalized(a => a.Name), pam.ValidationMinLength.Value));
                            warnings.Add("FieldError:product_attribute_" + pam.Id);
                        }
                    }
                }
                ...
                if (pam.ValidationMaxLength.HasValue)
                {
                    ...
                    if (pam.AttributeControlType == AttributeControlType.Checkboxes)
                    {
                        ...
                        if (valuesStr.Count > pam.ValidationMaxLength.Value)
                        {
                            warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.CheckboxMaximumLength"), pam.ProductAttribute.GetLocalized(a => a.Name), pam.ValidationMaxLength.Value));
                            warnings.Add("FieldError:product_attribute_" + pam.Id);
                        }
                    }
                }
            ...
            foreach (var attributeValue in attributeValues)
            {
                if (attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
                {
                    ...
                    if (associatedProduct != null)
                    {
                        ...
                        bool isParentBundleProduct = true;
                        ...
                        var associatedProductWarnings = GetShoppingCartItemWarnings(customer,
                            ...
                            "", decimal.Zero, null, null, totalQty, false, isParentBundleProduct);
        ...
        public virtual IList<string> GetShoppingCartItemWarnings(Customer customer, ShoppingCartType shoppingCartType,
            ...
            bool getMealPlanWarnings = true, bool getDonationWarnings = true, bool isParentBundleProduct = true)	/// NU-16, NU-17
        {
            if (product == null)
                throw new ArgumentNullException("product");
            ...
            if (getStandardWarnings)
                warnings.AddRange(GetStandardWarnings(customer, shoppingCartType, product, attributesXml, customerEnteredPrice, quantity,isParentBundleProduct));
            ...
            #region NU-16
            if (getMealPlanWarnings)
                warnings.AddRange(GetShoppingCartItemMealPlanWarnings(shoppingCartType, product, attributesXml));
            #endregion

            #region NU-17
            if (getDonationWarnings)
                warnings.AddRange(GetShoppingCartItemDonationWarnings(shoppingCartType, product, attributesXml));
            #endregion
            ...
            if (validateCheckoutAttributes)
            {
                ...
                foreach (var ca in attributes2)
                {
                    if (ca.ValidationMinLength.HasValue)
                    {
                        ...
                        if (ca.AttributeControlType == AttributeControlType.Checkboxes)
                        {
                            var valuesStr = _checkoutAttributeParser.ParseValues(checkoutAttributesXml, ca.Id);

                            if (valuesStr.Count < ca.ValidationMinLength.Value)
                            {
                                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.CheckboxMinimumLength"), ca.GetLocalized(a => a.Name), ca.ValidationMinLength.Value));
                            }
                        }
                    }
                    ...
                    if (ca.ValidationMaxLength.HasValue)
                    {
                        ...
                        if (ca.AttributeControlType == AttributeControlType.Checkboxes)
                        {
                            ...
                            if (ca.ValidationMaxLength.Value < valuesStr.Count)
                            {
                                warnings.Add(string.Format(_localizationService.GetResource("ShoppingCart.CheckboxMinimumLength"), ca.GetLocalized(a => a.Name), ca.ValidationMaxLength.Value));
                            }
                        }
                    }
                }
        ...
        public virtual IList<string> AddToCart(Customer customer, Product product,
            ...
            decimal customerEnteredPrice = decimal.Zero,
            DateTime? rentalStartDate = null, DateTime? rentalEndDate = null,
            int quantity = 1, DateTime? selectedReservationDate = null, string selectedTimeSlot = "", bool automaticallyAddRequiredProductsIfEnabled = true)
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            ...
            if (shoppingCartItem != null)
            {
                ...
                if (!warnings.Any())
                {
                    ...
                    shoppingCartItem.Quantity = newQuantity;
                    shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                    shoppingCartItem.IsBundleProduct = product.IsBundleProduct;
            ...
            else
            {
                ...
                if (!warnings.Any())
                {
                    ...
                    DateTime now = DateTime.UtcNow;
                    DateTime reservationDate = (product.IsReservation && selectedReservationDate.HasValue) ? selectedReservationDate.Value : DateTime.UtcNow;
                    shoppingCartItem = new ShoppingCartItem
                    {
                        ShoppingCartType = shoppingCartType,
                        StoreId = storeId,
                    ...
                    //ShoppingCartItem si = customer.ShoppingCartItems.Where(x => x.Id == 0).FirstOrDefault();
                    ...
                    decimal distributedPrice = (product.IsBundleProduct && attributeValues.Count > 0) ? Math.Round(product.Price / attributeValues.Count,4) : 0;
                    decimal roundedAmount = product.Price - (distributedPrice * attributeValues.Count);

                    Dictionary<int, Product> associatedProductDict = new Dictionary<int, Product>();
                    int counter = 0;
                    foreach (var attributeValue in attributeValues)
                    {
                        if (attributeValue.AttributeValueType == AttributeValueType.AssociatedToProduct)
                        {
                            var associatedProduct = _productService.GetProductById(attributeValue.AssociatedProductId);
                            ShoppingCartBundleProductItem scbpi = new ShoppingCartBundleProductItem();
                            decimal finalPrice = distributedPrice; 

                            scbpi.ShoppingCartItemId = shoppingCartItem.Id;
                            scbpi.ParentProductId = shoppingCartItem.ProductId;
                            scbpi.AssociatedProductId = associatedProduct.Id;
                            scbpi.Price = (counter == 0 ? finalPrice + roundedAmount : finalPrice);
                            scbpi.Quantity = attributeValue.Quantity;
                            scbpi.AssociatedProductName = attributeValues.Where(x => x.AssociatedProductId == associatedProduct.Id).FirstOrDefault().Name;
                            scbpi.AssociatedProductTaxCategoryId = associatedProduct.TaxCategoryId;
                            InsertAssociatedProductsPerShoppingCartItem(scbpi);

                            counter++;
                        }
                    }
                    System.Web.HttpContext.Current.Session["InsertedShoppingCartItemId"] = shoppingCartItem.Id;
                }
            }
        ...
        /// <param name="rentalStartDate">Rental start date</param>
        /// <param name="rentalEndDate">Rental end date</param>
        /// <param name="requestedFulfillmentDateTime">Date and Time that the consumer requests product.</param>
        ...
        public virtual IList<string> UpdateShoppingCartItem(Customer customer,
            ...
            DateTime? requestedFulfillmentDateTime = null, /// NU-13
            int? selectedWarehouseId = -1, 	/// NU-13
            string shippingMethodName = null)	/// NU-13
        {
            if (customer == null)
                throw new ArgumentNullException("customer");
            var warnings = new List<string>();
            ...
            if (shoppingCartItem != null)
            {
                ...
                {
                    #region NU-13
                    if (requestedFulfillmentDateTime != null)
                    {
                        //if everything is OK, then update a shopping cart item
                        shoppingCartItem.RequestedFullfilmentDateTime = requestedFulfillmentDateTime;
                        shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                    }
                    ...
                    {
                        //if everything is OK, then update a shopping cart item
                        shoppingCartItem.SelectedWarehouseId = selectedWarehouseId;
                        shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                    }
                    ...
                    {
                        //if everything is OK, then update a shopping cart item
                        shoppingCartItem.SelectedShippingMethodName = shippingMethodName;
                        shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                    }
                    //TODO implement Selected Shipping Method Rate
                    _customerService.UpdateCustomer(customer); //TODO:  Why do i need this, it's in the old system?
                    #endregion
                    ...
                    if (!warnings.Any())
                    {
                        ...
                        shoppingCartItem.UpdatedOnUtc = DateTime.UtcNow;
                        shoppingCartItem.IsBundleProduct = shoppingCartItem.Product.IsBundleProduct;
                        _customerService.UpdateCustomer(customer);
        ...
        public void UpdateShoppingCartItemForTieredShipping(ShoppingCartItem sci)
        {
            if (sci == null)
                throw new ArgumentNullException("ShoppingCartItem");
            ...
            if (sci != null)
            {
                ShoppingCartItem sc = _sciRepository.GetById(sci.Id);
                sc.IsTieredShippingEnabled = sci.IsTieredShippingEnabled;
                sc.FlatShipping = sci.FlatShipping;
                sc.IsContractShippingEnabled = sci.IsContractShippingEnabled;
                sc.RCNumber = sci.RCNumber;
                sc.IsInterOfficeDeliveryEnabled = sci.IsInterOfficeDeliveryEnabled;
                sc.MailStopAddress = sci.MailStopAddress;
                sc.IsFirstCartItem = sci.IsFirstCartItem;
                _sciRepository.Update(sci);
            }
        }

        public void UpdateShoppingCartBundleProductItem(ShoppingCartBundleProductItem scbp)
        {
            if (scbp == null)
                throw new ArgumentNullException("ShoppingCartBundleProductItem is null");

            if (scbp != null)
            {
                _scbpiRepository.Update(scbp);
            }
        }

        public void InsertAssociatedProductsPerShoppingCartItem(ShoppingCartBundleProductItem scbpi)
        {
            if (scbpi == null)
                throw new ArgumentNullException("ShoppingCartItem");
            ...
            if (scbpi != null)
            {
                _scbpiRepository.Insert(scbpi);
            }
        }

        public IList<ShoppingCartBundleProductItem> GetAssociatedProductsPerShoppingCartItem(int shoppingCartItemId)
        {
            if (shoppingCartItemId < 0)
                throw new ArgumentNullException("ShoppingCartItem Id is null");
            else
            {
               var query = from p in _scbpiRepository.Table
                            orderby p.Id
                            where p.ShoppingCartItemId == shoppingCartItemId
                            select p;
        ...
        public IList<ShoppingCartBundleProductItem> GetAssociatedProductsByOrderItemId(int orderItemId)
        {
            if (orderItemId < 0)
                throw new ArgumentNullException("ShoppingCartItem Id is null");
            else
        ...
        public IList<ShoppingCartBundleProductItem> GetAssociatedProductsByOrderId(int orderId)
        {
            if (orderId < 0)
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            {
                var sci = fromCart[i];
                DateTime reservationDate = sci.ReservationDate != null ? sci.ReservationDate : new DateTime(1900, 1, 1);
                string reservedTimeSlot = (!String.IsNullOrEmpty(sci.ReservedTimeSlot)) ? sci.ReservedTimeSlot : "";
                AddToCart(toCustomer,
                             sci.Product,
        ...
        public ShoppingCartItem GetShoppingCartItemById(int shoppingCartItemId)
        {
            ...
            else
            {
                var query = from p in _sciRepository.Table
                            orderby p.Id
                            where p.Id == shoppingCartItemId
                            select p;

                var associatedProducts = query.ToList();
                return associatedProducts.FirstOrDefault();
            }
        }

        #region NU-16
        /// <summary>
        /// Validates shopping cart item when it's a meal plan
        /// </summary>
        /// <param name="shoppingCartType">Shopping cart type</param>
        /// <param name="siteProduct">Site Product</param>
        /// <param name="selectedAttributes">Selected attributes</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> GetShoppingCartItemMealPlanWarnings(ShoppingCartType shoppingCartType, Product product, string selectedAttributes)
        {
            if (product == null)
                throw new ArgumentNullException("product");
            ...
            if (product.IsMealPlan && product.ShowStandardMealPlanFields)
            {
                ...
                {
                    _productAttributeParser.GetMealPlanAttribute(selectedAttributes,
                    out mealPlanRecipientAcctNum, out mealPlanRecipientAddress,
                    out mealPlanRecipientEmail, out mealPlanRecipientName, out mealPlanRecipientPhone);

                    if (string.IsNullOrEmpty(mealPlanRecipientAcctNum))
                    {
                        warnings.Add("Must enter a Account/ID Number (Student, Faculty, Staff, etc.).");
                        warnings.Add("FieldError:" + string.Format("mealplan_{0}.MealPlanRecipientAcctNum", product.Id));
                    }
                    if (string.IsNullOrEmpty(mealPlanRecipientName))
                    {
                        warnings.Add("Must enter the recipient's name.");
                        warnings.Add("FieldError:" + string.Format("mealplan_{0}.MealPlanRecipientName", product.Id));
                    }
                    if (string.IsNullOrEmpty(mealPlanRecipientAddress))
                    {
                        warnings.Add("Must enter an address.");
                        warnings.Add("FieldError:" + string.Format("mealplan_{0}.MealPlanRecipientAddress", product.Id));
                    }
                    if (string.IsNullOrEmpty(mealPlanRecipientPhone) || !CommonHelper.IsValidPhoneNumber(mealPlanRecipientPhone))
                    {
                        warnings.Add("Must enter a valid phone number.");
                        warnings.Add("FieldError:" + string.Format("mealplan_{0}.MealPlanRecipientPhone", product.Id));
                    }
                    if (string.IsNullOrEmpty(mealPlanRecipientEmail) || !CommonHelper.IsValidEmail(mealPlanRecipientEmail))
                    {
                        warnings.Add("Must enter a valid email address.");
                        warnings.Add("FieldError:" + string.Format("mealplan_{0}.MealPlanRecipientEmail", product.Id));
                    }
        ...
        /// <param name="selectedAttributes">Selected attributes</param>
        /// <returns>Warnings</returns>
        public virtual IList<string> GetShoppingCartItemDonationWarnings(ShoppingCartType shoppingCartType, Product product, string selectedAttributes)
        {
            if (product == null)
                throw new ArgumentNullException("Product");
            ...
            if (product.IsDonation)
            {
                ...
                _productAttributeParser.GetDonationAttribute(selectedAttributes,
                ...
                {
                    warnings.Add("Must enter the donor's first name.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorFirstName", product.Id));
                }
                if (string.IsNullOrEmpty(donorLastName))
                {
                    warnings.Add("Must enter the donor's last name.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorLastName", product.Id));
                }
                if (string.IsNullOrEmpty(donorAddress))
                {
                    warnings.Add("Must enter the donor's street address");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorAddress", product.Id));
                }
                if (string.IsNullOrEmpty(donorCity))
                {
                    warnings.Add("Must enter the donor's city.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorCity", product.Id));
                }
                if (string.IsNullOrEmpty(donorState))
                {
                    warnings.Add("Must enter the donor's state.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorState", product.Id));
                }
                if (string.IsNullOrEmpty(donorZip))
                {
                    warnings.Add("Must enter the donors zip code.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorZip", product.Id));
                }
                if (string.IsNullOrEmpty(donorPhone))
                {
                    warnings.Add("Must enter the donor's phone number.");
                    warnings.Add("FieldError:" + string.Format("donation_{0}.DonorPhone", product.Id));
                }

                if (!string.IsNullOrEmpty(notificationEmail))
                {
                    if (!CommonHelper.IsValidEmail(notificationEmail))
                    {
                        warnings.Add("Must enter a valid notification email address.");
                        warnings.Add("FieldError:" + string.Format("donation_{0}.NotificationEmail", product.Id));
                    }
                }
            }
