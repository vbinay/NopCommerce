...
namespace Nop.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        public virtual IList<Warehouse> GetAllWarehouses(
            int storeId = -1, /// NU-29
            int vendorId = -1, /// NU-29
            int customerId = -1 /// NU-29
            )
        {
            #region NU-29
            ...
            if (customerId > 0)
            {
                var stores = (from sm in _storeMappingRepository.Table
                              where sm.EntityName == "Customer" && sm.EntityId == customerId
                              select sm.StoreId).Distinct().ToList();

                query = from w in query
                        join sm in _storeMappingRepository.Table
                        on new { c1 = w.Id, c2 = "Warehouse" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into w_sm
                        from sm in w_sm
                        where stores.Contains(sm.StoreId)
                        select w;
            }
            else if (storeId > 0 && !_catalogSettings.IgnoreStoreLimitations)
            {
                //Store mapping
                query = from bp in query
                        join sm in _storeMappingRepository.Table
                        on new { c1 = bp.Id, c2 = "Warehouse" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into bp_sm
            ...
            {
                query = from w in query
                        join vm in _vendorMappingRepository.Table
                        on new { c1 = w.Id, c2 = "Warehouse" } equals new { c1 = vm.EntityId, c2 = vm.EntityName } into w_vm
                        from vm in w_vm.DefaultIfEmpty()
                        where vendorId == vm.VendorId
                        select w;
            }
            else if (storeId == 0)
            {
        ...
        public virtual decimal GetTotalWeight(GetShippingOptionRequest request, bool includeCheckoutAttributes = true)
        {
            ...
            foreach (var packageItem in request.Items)
                totalWeight += GetShoppingCartItemWeight(packageItem.ShoppingCartItem) * 1;//Setting this to 1 quantity so as to send only the request for a sigle product then we shall multiply the actual rate with the quantity
            ...
            out decimal width, out decimal length, out decimal height)
        {
            ...
            if (_shippingSettings.UseCubeRootMethod)
            {
                ...
                foreach (var packageItem in packageItems)
                {
                    ...
                    var qty = 1;//always set this to 1 so that we dont have any upgradation of the dimesnion based on the qunatity
        ...
        public virtual Warehouse GetNearestWarehouse(Address address, IList<Warehouse> warehouses = null,
            int storeId = 0)	/// NU-42
        {
            warehouses = warehouses ?? GetAllWarehouses(storeId);	/// NU-42
            ...
            Address shippingAddress, int storeId, out bool shippingFromMultipleLocations)
        {
            ...
            {
                if (!sci.Product.IsShipEnabled && !sci.Product.IsPickupEnabled)	/// NU-13
                    continue;
        ...
        public virtual List<string> CreateShipmentWithUPSAPI(OrderItem orderItem)
        {
            var result = new List<string>();
            //ups object
            var shippingRateComputationMethods = LoadActiveShippingRateComputationMethods(orderItem.Order.StoreId);

            foreach (var srcm in shippingRateComputationMethods)
            {
                result = srcm.CreateShipmentWithUPSAPI(orderItem, _addressService, _shippingSettings);
                if (result.Count == 0)
                    continue;
                else if (result[0].Contains("CreateShipmentWithUPSAPI.NotImplemented"))
                    continue;
                else
                    break;
            }
        ...
        //---START: Codechages done by (na-sdxcorp\ADas)--------------
        public virtual List<ShipmentReport> GetShipmentReports(IList<Order> orders)
        {
            //Populate all ShipmentReports from orders
            var shipmentReports = new List<ShipmentReport>();
            foreach (var ord in orders)
            {
                //If the order has 1 or more shipments then loop through all shipments
                if (ord.Shipments.Count > 0)
                {
                    foreach (var shpmnt in ord.Shipments)
                    {
                        shipmentReports.Add(new ShipmentReport
                        {
                            OrderId = ord.Id,
                            StoreId = ord.StoreId,
                            OrderStatus = ord.OrderStatus.ToString(),
                            OrderTotal = ord.OrderTotal,
                            ShipmentID = shpmnt.Id,
                            ShippingMethod = ord.ShippingMethod,
                            ShippingStatus = ord.ShippingStatus.ToString(),
                            ShipmentTrackingNumber = shpmnt.TrackingNumber,
                            ShippingCost = ord.OrderShippingInclTax,
                            RecipientContactInfo = ord.ShippingAddress.PhoneNumber,
                            RecipientName = ord.ShippingAddress.FirstName + " " + ord.ShippingAddress.LastName,
                            RecipientCountry = ord.ShippingAddress.Country.Name,
                            RecipientStateProvince = ord.ShippingAddress.StateProvince.Name,
                            RecipientCity = ord.ShippingAddress.City,
                            RecipientAddress1 = ord.ShippingAddress.Address1,
                            RecipientAddress2 = ord.ShippingAddress.Address2,
                            RecipientZipPostalCode = ord.ShippingAddress.ZipPostalCode
                        });
                    }
                }
                //If the order has no shipments then put default values in shipment specific items below
                else
                {
                    shipmentReports.Add(new ShipmentReport
                    {
            ...
            int storeId = 0)
        {
            ...
            foreach (var srcm in shippingRateComputationMethods)
            {
                ...
                foreach (var shippingOptionRequest in shippingOptionRequests)
                {
                    ...
                    {
                        var upsShippingOptions = getShippingOptionResponse.ShippingOptions.Where(x => x.Name.Contains("UPS")).ToList();
                        if (upsShippingOptions.Count > 0 && upsShippingOptions != null)
                        {
                ...
                if (srcmShippingOptions != null)
                {
                    if (srcmShippingOptions.Where(x => x.Name.Contains("UPS")).ToList().Count <=0)
                    {
                        foreach (var so in srcmShippingOptions)
                        {
                            //set system name if not set yet
            ...
            //option.Description = "FlatRateShipping";
            //option.Name = "FlatRateShipping";
            //option.Rate = 0;
            //option.RequireFulfilmentCalendar = false;
            //option.ShippingRateComputationMethodSystemName = "FlatRateShipping";
            //option.ShoppingCartProductId = 0;
