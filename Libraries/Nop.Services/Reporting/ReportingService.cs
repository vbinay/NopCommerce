...
namespace Nop.Services.Orders
{
    ...
    public partial class ReportingService : IReportingService
    {
        ...
        private readonly IRepository<ProductGlCodeMapping> _productGlcodeMappingRepository;
        private readonly IRepository<CovidRefunds> _covidRefundsRepository;
        ...
        public ReportingService(IRepository<Order> orderRepository,
            ...
            IRepository<ProductGlCodeMapping> productGlcodeMappingRepository,
            IRepository<CovidRefunds> covidRefundsRepository)
        {
            ...
            this._productGlcodeMappingRepository = productGlcodeMappingRepository;
            this._covidRefundsRepository = covidRefundsRepository;
        }
        ...
        public IList<OrderExcelExport> GetAllJournalRecords(DateTime? startTime, DateTime? endTime)
        {
            var query = (from s in _storeRepository.Table
                         ...
                         join OT in this._orderItemRepository.Table on OD.Id equals OT.OrderId
                         ...
                         where Glc.Processed == false && Glc.GlStatusType == 1
                         && (OD.PaidDateUtc >= startTime && OD.PaidDateUtc <= endTime) || (OT.DateOfRefund >= startTime && OT.DateOfRefund <= endTime) && !OD.Deleted
                         select new OrderExcelExport()
                         {
        ...
        public IList<OrderExcelExport> GetStoreSpecificJournalRecords(DateTime? startTime, DateTime? endTime, int storeId)
        {
            var query = (from s in _storeRepository.Table
                         ...
                         join sp1 in this._stateProvinceRepository.Table on AD.StateProvinceId equals sp1.Id
                         join P in this._productRepository.Table on Glc.ProductId equals P.Id
                         /* join GN in this._genericRepository.Table on Glc.OrderId equals GN.EntityId*/ // REMOVE THIS
                         join GL in this._glCode.Table on Glc.GLCodeId equals GL.Id into abc
                         from data in abc.DefaultIfEmpty()
            ...
            if (productInfo.VendorId != 0) //Checking for Vendor Product
            {
                ...
                if (UpdateDetailsWithAmountAndGlFromIncomingRequest.Amount != 0)
                {
                    ...
                    if (OrderDetails.OrderStatusId == 40 && !orderitems.IsPartialRefund && orderitems.FulfillmentDateTime != null)// Full Refund of the Product if Fulfilled
                    {
                        ...
                        {
                            decimal? AmountReturned = ApplyDeliveryReportShippingRules(productInfo, orderitems, codes).FirstOrDefault()?.Amount;
                            if (AmountReturned != null && AmountReturned.Value != 0)
                            {
                                returnValue.Add(new OrderExcelExport
                                {
                                    SiteExtKey = UpdateDetailsWithAmountAndGlFromIncomingRequest.SiteExtKey, //1
        ...
        public IList<OrderExcelExport> ApplyPartialRefundBusinessRules(Product productInfo, OrderItem orderitems, Order OrderDetails)
        {
            ...
            if (orderitems.FulfillmentDateTime == null) // Partial refund data capture for Non-Fulfilled Order Items against the Base Gl of 2040 (Non Vendor) and 2041 (Vendor)
            {
                ...
                if (!string.IsNullOrEmpty(orderitems.TaxName1) && orderitems.TaxName1 != "None")
                {
                    if (!partialTaxAndGlDetails.ContainsKey(Convert.ToString(orderitems.TaxName1.Split('-')[0])))
                    {
                        partialTaxAndGlDetails.Add(Convert.ToString(orderitems.TaxName1.Split('-')[0]), orderitems.RefundedTaxAmount1);
                    }
                }
                ...
                returnList.Add(new OrderExcelExport
                {
                    ...
                    paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund,//11
                    PaymentType = UpdateDetailsWithAmountAndGlFromIncomingRequest.PaymentType,//12
                    VertexTaxAreaId = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShiptoState + UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipToZipCode,//13
                    SiteName = UpdateDetailsWithAmountAndGlFromIncomingRequest.SiteName,
            ...
            else if (orderitems.FulfillmentDateTime != null) // Partial Refund for Items which has been Fulfuilled
            {
                ...
                if (!string.IsNullOrEmpty(orderitems.TaxName1) && orderitems.TaxName1 != "None" && (orderitems.RefundedTaxAmount1 != null && orderitems.RefundedTaxAmount1 != 0))
                {
                    if (!partialRefundTaxDetailsFulfilled.ContainsKey(Convert.ToString(orderitems.TaxName1 + "_1")))
                    {
                        partialRefundTaxDetailsFulfilled.Add(Convert.ToString(orderitems.TaxName1.Split('-')[0]), orderitems.RefundedTaxAmount1);
                    }
                }
        ...
        // For the New JE Reports
        public IList<OrderExcelExport> ApplyPartialRefundBusinessRules(Product productInfo, OrderItem orderitems, Order OrderDetails, OrderExcelExport UpdateDetailsWithAmountAndGlFromIncomingRequest)
        {
            var returnList = new List<OrderExcelExport>();
            ...
            if (orderitems.FulfillmentDateTime == null) // Partial refund data capture for Non-Fulfilled Order Items against the Base Gl of 2040 (Non Vendor) and 2041 (Vendor)
            {
                ...
                if (!string.IsNullOrEmpty(orderitems.TaxName1) && orderitems.TaxName1 != "None")
                {
                    var refundedAmount = orderitems.RefundedTaxAmount1;
                    if (orderitems.RefundedTaxAmount1 == null && orderitems.RefundedTaxAmount2.HasValue && orderitems.TaxName1 != orderitems.TaxName2)
                    {
                        // I think this is actually user error, but let's figure it out after we solve the issue observed.
                        // The user error seems consistent which means we can correct against it until we isolate it.
                        refundedAmount = orderitems.RefundedTaxAmount2;
                    }
                    if (!partialTaxAndGlDetails.ContainsKey(Convert.ToString(orderitems.TaxName1.Split('-')[0])))
                    {
                        partialTaxAndGlDetails.Add(Convert.ToString(orderitems.TaxName1.Split('-')[0]), refundedAmount);
                    }
                }
                if (!string.IsNullOrEmpty(orderitems.TaxName2) && orderitems.TaxName2 != "None") { if (!partialTaxAndGlDetails.ContainsKey(Convert.ToString(orderitems.TaxName2.Split('-')[0]))) { partialTaxAndGlDetails.Add(Convert.ToString(orderitems.TaxName2.Split('-')[0]), orderitems.RefundedTaxAmount2); } }
                if (!string.IsNullOrEmpty(orderitems.TaxName3) && orderitems.TaxName3 != "None") { if (!partialTaxAndGlDetails.ContainsKey(Convert.ToString(orderitems.TaxName3.Split('-')[0]))) { partialTaxAndGlDetails.Add(Convert.ToString(orderitems.TaxName3.Split('-')[0]), orderitems.RefundedTaxAmount3); } }
                ...
                returnList.Add(new OrderExcelExport
                {
                    ...
                    storeName = UpdateDetailsWithAmountAndGlFromIncomingRequest.storeName,
                    storeSate = UpdateDetailsWithAmountAndGlFromIncomingRequest.storeSate,
                    storeZip = UpdateDetailsWithAmountAndGlFromIncomingRequest.storeZip,
                    StoreCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.StoreCity,
                    LegalEntity = UpdateDetailsWithAmountAndGlFromIncomingRequest.LegalEntity,
                    ShipFromState = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromState,
                    ShipFromZipCode = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromZipCode,
                    ShipFromCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromCity,
                    ShiptoState = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShiptoState,
                    ShipToZipCode = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipToZipCode,
                    ShippedToCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShippedToCity,
                    OrderId = UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderId,
                    ProductName = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductName + "-Partially-Refunded-Product-Amount",
                    paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund,
                    ProductId = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductId,
                    VertexTaxAreaId = UpdateDetailsWithAmountAndGlFromIncomingRequest.VertexTaxAreaId,
                    isPartialRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund,
                    isFullRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isFullRefund,
                    Amount = -(orderitems.RefundedGlAmount1),
                    GLCode = productInfo.VendorId != 0 ? "48702041" : "48702040",
                ...
                if (orderitems.IsDeliveryPickUp && orderitems.IsFullRefund)
                {
                    if (orderitems.DeliveryPickupAmount != decimal.Zero)
                    {
                        returnList.Add(new OrderExcelExport
                        {
                            SiteExtKey = UpdateDetailsWithAmountAndGlFromIncomingRequest.SiteExtKey,
                            ...
                            ShippedToCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShippedToCity,
                            OrderId = UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderId,
                            ProductName = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductName + "-Partially-Refunded-Product-Delivery/Shipping Amount",
                            paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund,
                            ProductId = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductId,
                            VertexTaxAreaId = UpdateDetailsWithAmountAndGlFromIncomingRequest.VertexTaxAreaId,
                            isPartialRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund,
                ...
                if (orderitems.IsShipping && orderitems.IsFullRefund)
                {
                    if (orderitems.ShippingAmount != decimal.Zero)
                    {
                        returnList.Add(new OrderExcelExport
                        {
                            SiteExtKey = UpdateDetailsWithAmountAndGlFromIncomingRequest.SiteExtKey,
                ...
                foreach (var items in partialTaxAndGlDetails)
                {
                    if (items.Value != 0)
                    {
                        ...
                        returnList.Add(new OrderExcelExport
                        {
                            ...
                            ShippedToCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShippedToCity,
                            OrderId = UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderId,
                            ProductName = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductName + "-Partially-Refunded-Product-Tax",
                            paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund,
                            ProductId = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductId,
                            VertexTaxAreaId = UpdateDetailsWithAmountAndGlFromIncomingRequest.VertexTaxAreaId,
                            isPartialRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund,
                ...
                foreach (var delvShipTax in DeliveryTaxAndGlDetails)
                {
                    if (delvShipTax.Value != 0)
                    {
                        ...
                        returnList.Add(new OrderExcelExport
                        {
                            ...
                            ShippedToCity = UpdateDetailsWithAmountAndGlFromIncomingRequest.ShippedToCity,
                            OrderId = UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderId,
                            ProductName = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductName + "-Partially-Refunded-Product-Delivery/Shipping Tax",
                            paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund,
                            ProductId = UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductId,
                            VertexTaxAreaId = UpdateDetailsWithAmountAndGlFromIncomingRequest.VertexTaxAreaId,
                            isPartialRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund,
            ...
            else if (orderitems.FulfillmentDateTime != null) // Partial Refund for Items which has been Fulfuilled
            {
                ...
                var partialRefundDelvShipTaxDetailsFulfilled = new Dictionary<string, decimal?>();
                if (!string.IsNullOrEmpty(orderitems.TaxName1) && orderitems.TaxName1 != "None"
                    && (orderitems.RefundedTaxAmount1 != null && orderitems.RefundedTaxAmount1 != 0)
                    || (orderitems.RefundedTaxAmount1 == null && orderitems.RefundedTaxAmount2 != null && orderitems.RefundedTaxAmount2 != 0))
                {
                    var refundAmount = orderitems.RefundedTaxAmount1;
                    if (orderitems.RefundedTaxAmount1 == null && orderitems.RefundedTaxAmount2.HasValue && orderitems.TaxName1 != orderitems.TaxName2)
                    {
                        // I think this is actually user error, but let's figure it out after we solve the issue observed.
                        // The user error seems consistent which means we can correct against it until we isolate it.
                        refundAmount = orderitems.RefundedTaxAmount2;

                    }

                    if (!partialRefundTaxDetailsFulfilled.ContainsKey(Convert.ToString(orderitems.TaxName1 + "_1")) && refundAmount != null)
                    {
                        partialRefundTaxDetailsFulfilled.Add(Convert.ToString(orderitems.TaxName1.Split('-')[0]), refundAmount);
                    }
                }
                if (!string.IsNullOrEmpty(orderitems.TaxName2) && orderitems.TaxName2 != "None" && (orderitems.RefundedTaxAmount2 != null && orderitems.RefundedTaxAmount2 != 0)) { if (!partialRefundTaxDetailsFulfilled.ContainsKey(Convert.ToString(orderitems.TaxName2 + "_2"))) { partialRefundTaxDetailsFulfilled.Add(Convert.ToString(orderitems.TaxName2.Split('-')[0]), orderitems.RefundedTaxAmount2); } }
                if (!string.IsNullOrEmpty(orderitems.TaxName3) && orderitems.TaxName3 != "None" && (orderitems.RefundedTaxAmount3 != null && orderitems.RefundedTaxAmount3 != 0)) { if (!partialRefundTaxDetailsFulfilled.ContainsKey(Convert.ToString(orderitems.TaxName3 + "_3"))) { partialRefundTaxDetailsFulfilled.Add(Convert.ToString(orderitems.TaxName3.Split('-')[0]), orderitems.RefundedTaxAmount3); } }
                ...
                foreach (var items in PartialRefundGlDetailsFulfilled) //Adding all the Revenue GL in the Report once Partially Fulfilled
                {
                    returnList.Add(new OrderExcelExport
                    {
                        ...
                        isPartialRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund,
                        isFullRefund = UpdateDetailsWithAmountAndGlFromIncomingRequest.isFullRefund,
                        OrderItemId = UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderItemId,
                        Amount = -items.Value, // setting the tax amount refunded 
                        GLCode = items.Key.Split('-')[0], // setting the tax GL against which the tax is refunded
                    });
                }
        ...
        public IList<OrderExcelExport> ApplyJournalEntryBusinessRules(IList<OrderExcelExport> journalReps)
        {
            ...
            foreach (var itemstoProcessForReports in disticntOrdersToProcess)
            {
                ...
                UpdateDetailsWithAmountAndGlFromIncomingRequest.PaymentType = cardType;
        ...
        #region JE Reports
        /// <summary>
        /// JE Payment Report Per Store
        /// </summary>
        /// <param name="startTime"></param>
        /// <param name="endTime"></param>
        /// <param name="storeId"></param>
        /// <returns></returns>
        public IList<OrderExcelExport> JournalPaymentReportForExcelPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            var journalPaymentRecordsPerstore = GetAllJournalPaymentRecordsPerStore(startTime, endTime, storeId);
            //var startDate = startTime.Value.ToShortDateString();
            //var EndDate = endTime.Value.ToShortDateString();
            //bool isPaidStatus, isRefundedStatus;
            //string RefundDate = string.Empty;
            //List<string> RefundDateForPr = new List<string>();

            //List<OrderExcelExport> dataToremove = journalPaymentRecordsPerstore.Where(x => x.PaymentStatusId == 40 || x.isFullRefund || x.isPartialRefund).ToList();
            //if (dataToremove.Any())
            //{

            //    var distinctOrderid = dataToremove.Select(x => x.OrderId).Distinct();
            //    foreach (var orderidToProcessForRefunds in distinctOrderid)
            //    {
            //        var itemDetails = journalPaymentRecordsPerstore.Where(x => x.OrderId == orderidToProcessForRefunds).ToList();
            //        var paymentDateForItem = itemDetails.Select(x => x.PaidDate).FirstOrDefault();
            //        if (itemDetails.Where(x => x.PaymentStatusId == 40).Any())
            //        {
            //            RefundDate = itemDetails.Where(y => y.RefundedDate != "1/1/1988").Select(x => x.RefundedDate).FirstOrDefault();
            //            ReturnPaymentStatus(paymentDateForItem, RefundDate, startDate, EndDate, out isPaidStatus, out isRefundedStatus);
            //            if (isPaidStatus == false && isRefundedStatus == true)
            //            {
            //                var removeItems = itemDetails.Where(x => x.Amount >= 0).ToList();
            //                //Removing each items from the list
            //                foreach (var items in removeItems)
            //                    journalPaymentRecordsPerstore.Remove(items);

            //                foreach (var itemtoChange in journalPaymentRecordsPerstore.Where(x => x.Amount <= 0 && x.OrderId == orderidToProcessForRefunds))
            //                {
            //                    itemtoChange.paymentDate = null;
            //                }
            //            }
            //            if (isPaidStatus == true && isRefundedStatus == false)
            //            {
            //                var removeItems = itemDetails.Where(x => x.Amount <= 0).ToList();
            //                //Removing each items from the list
            //                foreach (var items in removeItems)
            ...
            //                foreach (var itemtoChange in journalPaymentRecordsPerstore.Where(x => x.Amount >= 0 && x.OrderId == orderidToProcessForRefunds))
            //                {
            //                    itemtoChange.DateOfRefund = null;
            //                }
            //            }
            //        }
            //        else
            //        {
            //            RefundDateForPr = itemDetails.Where(y => y.RefundedDate != "1/1/1988").Select(x => x.RefundedDate).Distinct().ToList();
            //            foreach (var refForPr in RefundDateForPr)
            //            {
            //                ReturnPaymentStatus(paymentDateForItem, refForPr, startDate, EndDate, out isPaidStatus, out isRefundedStatus);
            //                if (isPaidStatus == false && isRefundedStatus == true)
            //                {
            //                    var removeItems = itemDetails.Where(x => x.Amount >= 0 && x.RefundedDate == refForPr).ToList();
            //                    //Removing each items from the list
            //                    foreach (var items in removeItems)
            //                        journalPaymentRecordsPerstore.Remove(items);
            ...
            //                    {
            //                        itemtoChange.paymentDate = null;
            //                        itemtoChange.DateOfRefund = Convert.ToDateTime(refForPr);
            //                    }
            //                }
            //                if (isPaidStatus == true && isRefundedStatus == false)
            //                {
            ...
            //    }
            //}
            return journalPaymentRecordsPerstore;
        }
        public static void ReturnPaymentStatus(string paidDate, string RefundDate, string startDate, string endDate, out bool isPaidStatus, out bool isRefundedStatus)
        {
            var paymentDateofOrder = DateTime.Parse(paidDate);
            var RefundateofOrder = DateTime.Parse(RefundDate);
            var StartDateRange = DateTime.Parse(startDate);
            var EndDateRange = DateTime.Parse(endDate);
            isPaidStatus = false;
            isRefundedStatus = false;
            if (paymentDateofOrder >= StartDateRange && paymentDateofOrder <= EndDateRange && RefundateofOrder >= StartDateRange && RefundateofOrder <= EndDateRange)
            {

                isPaidStatus = true;
                isRefundedStatus = true;
            }
            if ((paymentDateofOrder >= StartDateRange && paymentDateofOrder <= EndDateRange) && (RefundateofOrder > EndDateRange || RefundateofOrder < StartDateRange))
            {
                isPaidStatus = true;
                isRefundedStatus = false;
            }
            if ((RefundateofOrder >= StartDateRange && RefundateofOrder <= EndDateRange) && (paymentDateofOrder > EndDateRange || paymentDateofOrder < StartDateRange))
            {
                isPaidStatus = false;
                isRefundedStatus = true;
            }
        ...
        /// <summary>
        /// JE Payment Report All Store
        /// </summary>
        /// <param name="startTime"></param>
        /// <param name="endTime"></param>
        /// <returns></returns>
        public IList<OrderExcelExport> JournalPaymentReportForExcelAllStore(DateTime? startTime, DateTime? endTime)
        {
            var journalPaymentRecordsAllstore = GetAllJournalPaymentRecordsAllStore(startTime, endTime);

            ////This Section is used in order to remove the Revenue data if purchase made earlier and is Refunded during the date in which the report is to be pulled
            //List<OrderExcelExport> dataToremove = journalPaymentRecordsAllstore.Where(x => x.paymentDate < startTime && x.DateOfRefund >= startTime && x.DateOfRefund <= endTime).ToList();
            //if (dataToremove.Any())
            //{
            //    //Removing Revenue Data from the Report
            //    dataToremove = dataToremove.Where(x => x.Amount >= 0).ToList();
            //    //Removing each items from the list
            //    foreach (var items in dataToremove)
            //        journalPaymentRecordsAllstore.Remove(items);
            //}
            return journalPaymentRecordsAllstore;
        }

        public IList<OrderExcelExport> JournalDeliveryForExcelPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            var JournalDeliveryReportPerStore = GetAllJournalDeliveryReportPerStore(startTime, endTime, storeId);
            return JournalDeliveryReportPerStore;
        }

        public IList<OrderExcelExport> JournalDeliveryForExcelAllStore(DateTime? startTime, DateTime? endTime)
        {
            var JournalDeliveryReportAllStore = GetAllJournalDeliveryReportAllStore(startTime, endTime);
            return JournalDeliveryReportAllStore;
        }


        #endregion
        public IList<OrderExcelExport> GetAllJournalPaymentRecordsPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            List<OrderExcelExport> finalrecords = new List<OrderExcelExport>();
            var query = (from O in _orderRepository.Table
                         join OT in this._orderItemRepository.Table on O.Id equals OT.OrderId
                         join OD in this._orderDetailsRepository.Table on new { a = OT.OrderId, b = OT.Id } equals new { a = OD.OrderId, b = OD.OrderItemId }
                         join PM in this._productGlcodeMappingRepository.Table on OD.OrderItemId equals PM.ProductID
                         where PM.GlStatusType == 1 && O.StoreId == storeId
                           && ((O.PaidDateUtc >= startTime && O.PaidDateUtc <= endTime) || (OT.DateOfRefund >= startTime && OT.DateOfRefund <= endTime)) && !O.Deleted && O.StoreId != 12
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
                             SiteExtKey = OD.StoreExtKey,
                             storeName = OD.StoreName,
                             storeSate = OD.StoreState,
                             storeZip = OD.StoreZip,
                             StoreCity = OD.StoreCity,
                             LegalEntity = OD.StoreLegalEntity,
                             ShipFromState = OD.ShipFromState,
                             ShipFromZipCode = OD.ShipFromZip,
                             ShipFromCity = OD.ShipFromCity,
                             ShiptoState = OD.ShipToState,
                             ShipToZipCode = OD.ShipToZip,
                             ShippedToCity = OD.ShipToCity,
                             GLCode = PM.Glcode,
                             Amount = PM.Amount,
                             OrderId = OD.OrderId,
                             ProductName = PM.ProductName,
                             paymentDate = O.PaidDateUtc,
                             ProductId = OT.ProductId,
                             VertexTaxAreaId = OD.VertexTaxAreaId,
                             isPartialRefund = OT.IsPartialRefund,
                             isFullRefund = OT.IsFullRefund,
                             OrderItemId = OT.Id,
                             DateOfRefund = OT.DateOfRefund,
                             PaymentStatusId = O.PaymentStatusId
                         });

            finalrecords = query.ToList();

            var dis = finalrecords.Where(x => x.isPartialRefund).GroupBy(x => x.OrderItemId).Select(y => y.First()).ToList(); // Fitering out the orders to check for partially refunded orders with orderitems that are partially refunded.

            if (dis.Any())
            {
                var UpdateDetailsWithAmountAndGlFromIncomingRequest = new OrderExcelExport();
                foreach (var dataItems in dis)
                {
                    var orderItems = _orderItemRepository.GetById(dataItems.OrderItemId);
                    var productinfo = _productRepository.GetById(dataItems.ProductId);
                    var orderDetails = _orderRepository.GetById(dataItems.OrderId);

                    if (orderItems.IsPartialRefund)
                    {

                        UpdateDetailsWithAmountAndGlFromIncomingRequest.SiteExtKey = dataItems.SiteExtKey;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.storeName = dataItems.storeName;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.storeSate = dataItems.storeSate;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.storeZip = dataItems.storeZip;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.StoreCity = dataItems.StoreCity;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.LegalEntity = dataItems.LegalEntity;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromState = dataItems.ShipFromState;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromZipCode = dataItems.ShipFromZipCode;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipFromCity = dataItems.ShipFromCity;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShiptoState = dataItems.ShiptoState;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShipToZipCode = dataItems.ShipToZipCode;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ShippedToCity = dataItems.ShippedToCity;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderId = dataItems.OrderId;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductName = dataItems.ProductName;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.paymentDate = dataItems.paymentDate;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.DateOfRefund = dataItems.DateOfRefund;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.ProductId = dataItems.ProductId;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.VertexTaxAreaId = dataItems.VertexTaxAreaId;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.isPartialRefund = dataItems.isPartialRefund;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.isFullRefund = dataItems.isFullRefund;
                        UpdateDetailsWithAmountAndGlFromIncomingRequest.OrderItemId = dataItems.OrderItemId;

                        // Adding up the Partial Refund records from the method
                        if (dataItems.DateOfRefund <= endTime && dataItems.DateOfRefund >= startTime)
                        {
                            finalrecords.AddRange(ApplyPartialRefundBusinessRules(productinfo, orderItems, orderDetails, UpdateDetailsWithAmountAndGlFromIncomingRequest));
                        }
                    }
                }

            }
            finalrecords.ForEach(x => x.PaidDate = x.paymentDate.Value.Date.ToShortDateString());
            finalrecords.ForEach(x => x.RefundedDate = x.DateOfRefund.HasValue ? x.DateOfRefund.Value.ToShortDateString() : "1/1/1988");

            // New order validation logic for JE Payments
            var verifiedOrders = new List<OrderExcelExport>();
            var excludedOrders = new List<OrderExcelExport>();
            foreach (var order in finalrecords)
            {
                if (order.paymentDate <= startTime || order.paymentDate >= endTime)
                {
                    // order should be only pulling refunded records.
                    if (order.DateOfRefund >= startTime && order.DateOfRefund <= endTime)
                    {
                        if (order.GLCode == "64701000")
                        {
                            if (order.Amount > 0)
                            {
                ...
                else // payment date is inside the range
                {
                    if (order.DateOfRefund != null && order.DateOfRefund <= startTime || order.DateOfRefund >= endTime)
                    {
                        // Refund records should be excluded
                        if (order.GLCode == "64701000")
                        {
                            // Negative Bonus GLs in this range are not refunded
                            if (order.Amount < 0)
                            {
                                verifiedOrders.Add(order);
                            }
                    ...
                    {
                        // Date of refund is not null and within range
                        // Payment date is within range
                        verifiedOrders.Add(order);
                    }
                }
            }
            // to compare against the old logic, you can uncomment these two lines and add quick watches.
            //var recordpullClearOriginalOrders = finalrecords.Where(x => ((x.paymentDate <= startTime && x.Amount < 0) || x.GLCode != "64701000") || x.paymentDate > startTime).ToList();
            //var recordpullFinal = recordpullClearOriginalOrders.Where(x => (x.paymentDate >= startTime && x.paymentDate <= endTime && x.Amount > 0) || 
            //(x.DateOfRefund >= startTime && x.DateOfRefund <= endTime && (x.Amount < 0 || (x.Amount > 0 && x.GLCode == "64701000"))) || x.DateOfRefund == null).ToList();
            return verifiedOrders.OrderByDescending(x => x.OrderId).OrderByDescending(x => x.OrderItemId).ToList();
        }

        public IList<OrderExcelExport> GetAllJournalPaymentRecordsAllStore(DateTime? startTime, DateTime? endTime)
        {
            List<OrderExcelExport> finalrecords = new List<OrderExcelExport>();
            var query = (from O in _orderRepository.Table
                         join OT in this._orderItemRepository.Table on O.Id equals OT.OrderId
                         join OD in this._orderDetailsRepository.Table on new { a = OT.OrderId, b = OT.Id } equals new { a = OD.OrderId, b = OD.OrderItemId }
                         join PM in this._productGlcodeMappingRepository.Table on OD.OrderItemId equals PM.ProductID
                         where PM.GlStatusType == 1
                         && ((O.PaidDateUtc >= startTime && O.PaidDateUtc <= endTime) || (OT.DateOfRefund >= startTime && OT.DateOfRefund <= endTime)) && !O.Deleted && O.StoreId != 12
                         //&& O.PaidDateUtc >= startTime && O.PaidDateUtc <= endTime && !O.Deleted
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
                             SiteExtKey = OD.StoreExtKey,
        ...
        public IList<OrderExcelExport> GetAllJournalDeliveryReportPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            var query = (from O in _orderRepository.Table
                         join OT in this._orderItemRepository.Table on O.Id equals OT.OrderId
                         join OD in this._orderDetailsRepository.Table on new { a = OT.OrderId, b = OT.Id } equals new { a = OD.OrderId, b = OD.OrderItemId }
                         join PM in this._productGlcodeMappingRepository.Table on OD.OrderItemId equals PM.ProductID
                         where PM.GlStatusType == 2 && O.StoreId == storeId
                         && OT.FulfillmentDateTime >= startTime && OT.FulfillmentDateTime <= endTime && !O.Deleted && O.StoreId != 12
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
                             SiteExtKey = OD.StoreExtKey,
        ...
        public IList<OrderExcelExport> GetAllJournalDeliveryReportAllStore(DateTime? startTime, DateTime? endTime)
        {
            var query = (from O in _orderRepository.Table
                         join OT in this._orderItemRepository.Table on O.Id equals OT.OrderId
                         join OD in this._orderDetailsRepository.Table on new { a = OT.OrderId, b = OT.Id } equals new { a = OD.OrderId, b = OD.OrderItemId }
                         join PM in this._productGlcodeMappingRepository.Table on OD.OrderItemId equals PM.ProductID
                         where PM.GlStatusType == 2
                         && OT.FulfillmentDateTime >= startTime && OT.FulfillmentDateTime <= endTime && !O.Deleted && O.StoreId != 12
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
        ...
        public IList<OrderExcelExport> GetAllStoresJournalDeliveryReportForExcel(DateTime? startTime, DateTime? endTime)
        {
            var query = (from s in _storeRepository.Table
                         ...
                         OT.FulfillmentDateTime >= startTime && OT.FulfillmentDateTime <= endTime && !OD.Deleted
                         select new OrderExcelExport()
                         {
                             Id = Glc.Id,
                             SiteExtKey = s.ExtKey,
        ...
        public virtual IList<OrderExcelExport> RefundsForExcel(int storeId, DateTime? startTime, DateTime? endTime, int pageIndex, int pageSize, TimeZoneInfo convertToTimeZone)
        {
            ...
            var query = (from o in query1
                         ...
                         from ga in gas.DefaultIfEmpty()
                         where orn.Note.Contains("refunded") && o.StoreId == storeId
                         //osp.DateOfRefund >= startTime.Value && osp.DateOfRefund <= endTime.Value
                         //&& 
        ...
        public virtual IList<OrderExcelExport> RefundsAllUnitsForExcel(DateTime? startTime, DateTime? endTime, int pageIndex, int pageSize, TimeZoneInfo convertToTimeZone)
        {
            ...
            var query = (from o in query1
                         ...
                         from ga in gas.DefaultIfEmpty()
                         where  orn.Note.Contains("refunded")
                         //osp.DateOfRefund >= startTime.Value && osp.DateOfRefund <= endTime.Value
                        // &&
