...
=======
                         join GN in this._genericRepository.Table on Glc.OrderId equals GN.EntityId
>>>>>>> feature/CharterTasks_after_11012019_release
                         join GL in this._glCode.Table on Glc.GLCodeId equals GL.Id into abc
                         from data in abc.DefaultIfEmpty()
        ...
        public IList<OrderExcelExport> ApplyPartialRefundBusinessRules(Product productInfo, OrderItem orderitems, Order OrderDetails, OrderExcelExport UpdateDetailsWithAmountAndGlFromIncomingRequest)
        {
            ...
            if (orderitems.FulfillmentDateTime == null) // Partial refund data capture for Non-Fulfilled Order Items against the Base Gl of 2040 (Non Vendor) and 2041 (Vendor)
            {
                ...
                returnList.Add(new OrderExcelExport
                {
                    ...
                    paymentDate = UpdateDetailsWithAmountAndGlFromIncomingRequest.paymentDate,
...
>>>>>>> feature/CharterTasks_after_11012019_release
        ...
        public IList<OrderExcelExport> GetAllJournalPaymentRecordsPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            ...
            return finalrecords.OrderByDescending(x => x.OrderId).OrderByDescending(x => x.OrderItemId).ToList();
        }
        ...
        public IList<OrderExcelExport> GetAllJournalPaymentRecordsAllStore(DateTime? startTime, DateTime? endTime)
        {
            ...
            var query = (from O in _orderRepository.Table
                         ...
                         join OD in this._orderDetailsRepository.Table on new { a = OT.OrderId, b = OT.Id } equals new { a = OD.OrderId, b = OD.OrderItemId }
                         join PM in this._productGlcodeMappingRepository.Table on OD.OrderItemId equals PM.ProductID
                         where PM.GlStatusType == 1 && O.PaidDateUtc >= startTime && O.PaidDateUtc <= endTime && !O.Deleted
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
                             SiteExtKey = OD.StoreExtKey,
        ...
        public IList<OrderExcelExport> GetAllJournalDeliveryReportPerStore(DateTime? startTime, DateTime? endTime, int storeId)
        {
            var query = (from O in _orderRepository.Table
                         ...
                         && OT.FulfillmentDateTime >= startTime && OT.FulfillmentDateTime <= endTime && !O.Deleted
                         orderby O.CreatedOnUtc descending
                         select new OrderExcelExport()
                         {
                             SiteExtKey = OD.StoreExtKey,
