            ...
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            ...
            if (storeId > 0 && !_catalogSettings.IgnoreStoreLimitations)
            {
                ...
                query = from bp in query
                        ...
                        where storeId == sm.StoreId /// NU-42
                        select bp;
            ...
            query = query.OrderByDescending(b => b.StartDateUtc ?? b.CreatedOnUtc);	/// NU-37
			...
			int storeId = 0) /// NU-42
        {
            ...
            if (storeId > 0)
            {
                query = from q in query
                        join sm in _storeMappingRepository.Table on q.BlogPostId equals sm.EntityId
                        where sm.EntityName == "BlogPost" && sm.StoreId == storeId
                        select q;
            }
            #endregion
