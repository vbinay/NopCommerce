...
namespace Nop.Services.Logging
{
    ...
    public partial class DefaultLogger : ILogger
    {
        ...
        public virtual Log ProductLastUpdatedBy(string productID, string isMaster, string store)
        {
            var query = _logRepository.Table;
            string conditions = "ProductID " + productID + " IsMaster " + isMaster;

            query = query.Where(l => l.ShortMessage.StartsWith(conditions));
            query = query.Where(l => l.PageUrl.Contains(store));                      
            query = query.OrderByDescending(l => l.CreatedOnUtc).Take(20);
