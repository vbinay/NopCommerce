    ...
    public partial class DiscountService : IDiscountService
    {
            ...
            int storeId = 0)	/// NU-42
        {
            ...
            var result = _cacheManager.Get(key, () =>
            {
                ...
                var discounts = query.Distinct().ToList();	// NU-45
                return discounts;
            });
        ...
        public virtual DiscountValidationResult ValidateDiscount(Discount discount, Customer customer, string couponCodeToValidate)
        {
            ...
            #region NU-65
            if(discount.LimitedToStores)
            {
                var coupon = from d in _discountRepository.Table
                             join sm in _storeMappingRepository.Table
                                 on d.Id equals sm.EntityId
                             where sm.EntityName == "Discount" &&
                                d.CouponCode.ToLower() == couponCodeToValidate.ToLower() &&
                                sm.StoreId == _storeContext.CurrentStore.Id
                             select d;
                if (coupon.Count() == 0)
                {
                    return result;
